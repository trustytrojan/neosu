...
CBaseUIContainerHorizontalBox::CBaseUIContainerHorizontalBox(float posX, float posY, float sizeX, float sizeY,
                                                             UString name)
    : CBaseUIContainerBase(name) {
    m_vPos.x = posX;
    m_vPos.y = posY;
    m_vmPos = m_vPos;
    m_vSize.x = sizeX;
...
void CBaseUIContainerHorizontalBox::updateLayout() {
    ...
    for(size_t i = 0; i < m_vElements.size(); i++) {
        if(!m_bWidthOverride && !m_bScaleByHeightOnly) {
            ...
            m_vElements[i]->setPosAbsolute(m_vPos.x + m_vMargin.x + (m_padding + widthStorage) * i,
                                           m_vPos.y + m_vMargin.y);
        }

        else {
            if(!m_bScaleByHeightOnly)
                m_vElements[i]->setSizeAbsolute(m_vSize.x * m_vElements[i]->getRelSize().x,
                                                m_vSize.y - 2 * m_vMargin.y);
            else
                m_vElements[i]->setSizeAbsolute(m_vSize.y / 9 * 16 * m_vElements[i]->getRelSize().x,
                                                m_vSize.y - 2 * m_vMargin.y);

            m_vElements[i]->setPosAbsolute(posStorage + (posStorage == m_vPos.x ? m_padding : m_vMargin.x),
                                           m_vPos.y + m_vMargin.y);
...
void CBaseUIContainerHorizontalBox::updateElement(CBaseUIElement *element) { updateLayout(); }
