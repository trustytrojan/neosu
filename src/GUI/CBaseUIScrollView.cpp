...
CBaseUIScrollView::CBaseUIScrollView(float xPos, float yPos, float xSize, float ySize, UString name)
    ...
    m_iScrollResistance = cv_ui_scrollview_resistance.getInt();  // TODO: dpi handling
    ...
    }
    {
        ...
        if(m_bDrawScrollbars) {
            ...
            if(m_bVerticalScrolling && m_vScrollSize.y > m_vSize.y) {
                ...
                if(((m_bScrollbarScrolling && m_bScrollbarIsVerticalScrolling) ||
                    m_verticalScrollbar.contains(engine->getMouse()->getPos())) &&
                   !m_bScrolling)
                    g->setAlpha(1.0f);

                g->fillRect(m_verticalScrollbar.getX(), m_verticalScrollbar.getY(), m_verticalScrollbar.getWidth(),
                            m_verticalScrollbar.getHeight());
                // g->fillRoundedRect(m_verticalScrollbar.getX(), m_verticalScrollbar.getY(),
                // m_verticalScrollbar.getWidth(), m_verticalScrollbar.getHeight(), m_verticalScrollbar.getWidth()/2);
            }
            ...
            if(m_bHorizontalScrolling && m_vScrollSize.x > m_vSize.x) {
                g->setColor(m_scrollbarColor);
                if(((m_bScrollbarScrolling && !m_bScrollbarIsVerticalScrolling) ||
                    m_horizontalScrollbar.contains(engine->getMouse()->getPos())) &&
                   !m_bScrolling)
                    g->setAlpha(1.0f);

                g->fillRect(m_horizontalScrollbar.getX(), m_horizontalScrollbar.getY(),
                            m_horizontalScrollbar.getWidth(), m_horizontalScrollbar.getHeight());
                // g->fillRoundedRect(m_horizontalScrollbar.getX(), m_horizontalScrollbar.getY(),
                // m_horizontalScrollbar.getWidth(), m_horizontalScrollbar.getHeight(),
...
void CBaseUIScrollView::mouse_update(bool *propagate_clicks) {
    ...
    if(m_bBusy) {
        ...
        anim->moveQuadOut(&m_vKineticAverage.x, deltaToAdd.x, cv_ui_scrollview_kinetic_approach_time.getFloat(), true);
        anim->moveQuadOut(&m_vKineticAverage.y, deltaToAdd.y, cv_ui_scrollview_kinetic_approach_time.getFloat(), true);
    ...
    if(m_bActive && !m_bBlockScrolling && (m_bVerticalScrolling || m_bHorizontalScrolling) && m_bEnabled) {
        ...
        if(m_container->isActive() && diff > m_iScrollResistance && !m_container->isBusy()) m_container->stealFocus();
        ...
        } else if(m_horizontalScrollbar.contains(engine->getMouse()->getPos()) && !m_bScrollbarScrolling &&
                  !m_bScrolling) {
            // NOTE: scrollbar dragging always force steals focus
            if(!wasContainerBusyBeforeUpdate) {
                m_container->stealFocus();
    ...
    if(!engine->getKeyboard()->isAltDown() && m_bMouseInside && m_bEnabled) {
        if(engine->getMouse()->getWheelDeltaVertical() != 0)
            scrollY(engine->getMouse()->getWheelDeltaVertical() * m_fScrollMouseWheelMultiplier *
                    cv_ui_scrollview_mousewheel_multiplier.getFloat());
        if(engine->getMouse()->getWheelDeltaHorizontal() != 0)
            scrollX(-engine->getMouse()->getWheelDeltaHorizontal() * m_fScrollMouseWheelMultiplier *
                    cv_ui_scrollview_mousewheel_multiplier.getFloat());
    }
    ...
    } else  // no longer scrolling, smooth the remaining velocity
    {
        ...
        if(!m_bAutoScrollingY && m_bVerticalScrolling) {
            ...
            } else if(std::round(std::abs(m_vScrollPos.y) + m_vSize.y) > m_vScrollSize.y &&
                      std::round(m_vScrollPos.y) < 1)  // rubber banding, bottom
            {
                anim->moveQuadOut(&m_vVelocity.y,
                                  (m_vScrollSize.y > m_vSize.y ? -m_vScrollSize.y : 1) +
                                      (m_vScrollSize.y > m_vSize.y ? m_vSize.y : 0),
                                  0.05f, 0.0f, true);
                anim->moveQuadOut(&m_vScrollPos.y, m_vVelocity.y, 0.2f, 0.0f, true);
            } else if(std::round(m_vVelocity.y) != 0 &&
                      std::round(m_vScrollPos.y) != std::round(m_vVelocity.y))  // kinetic scrolling
        ...
        if(!m_bAutoScrollingX && m_bHorizontalScrolling) {
            ...
            } else if(std::round(std::abs(m_vScrollPos.x) + m_vSize.x) > m_vScrollSize.x &&
                      std::round(m_vScrollPos.x) < 1)  // rubber banding, right
            {
                anim->moveQuadOut(&m_vVelocity.x,
                                  (m_vScrollSize.x > m_vSize.x ? -m_vScrollSize.x : 1) +
                                      (m_vScrollSize.x > m_vSize.x ? m_vSize.x : 0),
                                  0.05f, 0.0f, true);
                anim->moveQuadOut(&m_vScrollPos.x, m_vVelocity.x, 0.2f, 0.0f, true);
            } else if(std::round(m_vVelocity.x) != 0 &&
                      std::round(m_vScrollPos.x) != std::round(m_vVelocity.x))  // kinetic scrolling
    ...
    if(m_bScrollbarScrolling) {
        ...
        if(m_bScrollbarIsVerticalScrolling) {
            const float percent =
                clamp<float>((engine->getMouse()->getPos().y - m_vPos.y - m_verticalScrollbar.getWidth() -
                              m_verticalScrollbar.getHeight() - m_vMouseBackup.y - 1) /
                                 (m_vSize.y - 2 * m_verticalScrollbar.getWidth()),
                             0.0f, 1.0f);
            scrollToYInt(-m_vScrollSize.y * percent, true, false);
        } else {
            const float percent =
                clamp<float>((engine->getMouse()->getPos().x - m_vPos.x - m_horizontalScrollbar.getHeight() -
                              m_horizontalScrollbar.getWidth() - m_vMouseBackup.x - 1) /
                                 (m_vSize.x - 2 * m_horizontalScrollbar.getHeight()),
                             0.0f, 1.0f);
    ...
    // update scrollbars
    if(anim->isAnimating(&m_vScrollPos.y) || anim->isAnimating(&m_vScrollPos.x) || m_bScrolling ||
       m_bScrollbarScrolling)
        updateScrollbars();
...
void CBaseUIScrollView::onKeyUp(KeyboardEvent &e) { m_container->onKeyUp(e); }

void CBaseUIScrollView::onKeyDown(KeyboardEvent &e) { m_container->onKeyDown(e); }

void CBaseUIScrollView::onChar(KeyboardEvent &e) { m_container->onChar(e); }

void CBaseUIScrollView::scrollY(int delta, bool animated) {
    if(!m_bVerticalScrolling || delta == 0 || m_bScrolling || m_vSize.y >= m_vScrollSize.y || m_container->isBusy())
        return;
    ...
    if(animated && m_bAutoScrollingY) delta -= remainingVelocity;
    ...
    // clamp target
    {
        ...
        if(std::abs(target) + m_vSize.y > m_vScrollSize.y) {
            if(!allowOverscrollBounce)
                target = (m_vScrollSize.y > m_vSize.y ? -m_vScrollSize.y : m_vScrollSize.y) +
                         (m_vScrollSize.y > m_vSize.y ? m_vSize.y : 0);
...
void CBaseUIScrollView::scrollX(int delta, bool animated) {
    if(!m_bHorizontalScrolling || delta == 0 || m_bScrolling || m_vSize.x >= m_vScrollSize.x || m_container->isBusy())
        return;

    // TODO: fix all of this shit with the code from scrollY() above

    // stop any movement
    if(animated) m_vVelocity.x = 0;

    // keep velocity
    if(m_bAutoScrollingX && animated)
        delta += (delta > 0 ? (m_iPrevScrollDeltaX < 0 ? 0 : std::abs(delta - m_iPrevScrollDeltaX))
                            : (m_iPrevScrollDeltaX > 0 ? 0 : -std::abs(delta - m_iPrevScrollDeltaX)));
    ...
    if(std::abs(target) + m_vSize.x > m_vScrollSize.x)
        target = (m_vScrollSize.x > m_vSize.x ? -m_vScrollSize.x : m_vScrollSize.x) +
                 (m_vScrollSize.x > m_vSize.x ? m_vSize.x : 0);
...
void CBaseUIScrollView::scrollToX(int scrollPosX, bool animated) { scrollToXInt(scrollPosX, animated); }

void CBaseUIScrollView::scrollToY(int scrollPosY, bool animated) { scrollToYInt(scrollPosY, animated); }
...
void CBaseUIScrollView::updateScrollbars() {
    ...
    if(m_bVerticalScrolling && m_vScrollSize.y > m_vSize.y) {
        ...
        const float rawVerticalPercent =
            (m_vScrollPos.y > 0 ? -m_vScrollPos.y : std::abs(m_vScrollPos.y)) / (m_vScrollSize.y - m_vSize.y);
        ...
        m_verticalScrollbar =
            McRect(m_vPos.x + m_vSize.x - (verticalBlockWidth * m_fScrollbarSizeMultiplier),
                   m_vPos.y + (verticalPercent * (m_vSize.y - (verticalBlockWidth * 2) - verticalBlockHeight) +
                               verticalBlockWidth + 1),
                   (verticalBlockWidth * m_fScrollbarSizeMultiplier), verticalBlockHeight);
    }

    // update horizontal scrollbar
    if(m_bHorizontalScrolling && m_vScrollSize.x > m_vSize.x) {
        const float horizontalPercent = clamp<float>(
            (m_vScrollPos.x > 0 ? -m_vScrollPos.x : std::abs(m_vScrollPos.x)) / (m_vScrollSize.x - m_vSize.x), 0.0f,
            1.0f);
        ...
        m_horizontalScrollbar =
            McRect(m_vPos.x + (horizontalPercent * (m_vSize.x - (horizontalBlockWidth * 2) - horizontalBlockHeight) +
                   ...
                   m_vPos.y + m_vSize.y - horizontalBlockWidth, horizontalBlockHeight, horizontalBlockWidth);
    }
}

CBaseUIScrollView *CBaseUIScrollView::setScrollSizeToContent(int border) {
    auto oldScrollPos = m_vScrollPos;
    bool wasAtBottom = (m_vSize.y - m_vScrollPos.y) >= m_vScrollSize.y;
    ...
    for(size_t i = 0; i < elements.size(); i++) {
        ...
        if(x > m_vScrollSize.x) m_vScrollSize.x = x;
        if(y > m_vScrollSize.y) m_vScrollSize.y = y;
    }
    ...
    if(m_bVerticalScrolling && m_vScrollSize.y < m_vSize.y && m_vScrollPos.y != 1) scrollToY(1);
    if(m_bHorizontalScrolling && m_vScrollSize.x < m_vSize.x && m_vScrollPos.x != 1) scrollToX(1);

    updateScrollbars();

    if(wasAtBottom && sticky && !m_bFirstScrollSizeToContent) {
    ...
    m_bFirstScrollSizeToContent = false;
    return this;
}

void CBaseUIScrollView::scrollToLeft() { scrollToX(0); }

void CBaseUIScrollView::scrollToRight() { scrollToX(-m_vScrollSize.x); }

void CBaseUIScrollView::scrollToBottom() { scrollToY(-m_vScrollSize.y); }

void CBaseUIScrollView::scrollToTop() { scrollToY(0); }

void CBaseUIScrollView::onMouseDownOutside() { m_container->stealFocus(); }
...
void CBaseUIScrollView::onMouseUpInside() { m_bBusy = false; }

void CBaseUIScrollView::onMouseUpOutside() { m_bBusy = false; }
...
void CBaseUIScrollView::onEnabled() { m_container->setEnabled(true); }
...
bool CBaseUIScrollView::isBusy() { return (m_container->isBusy() || m_bScrolling || m_bBusy) && m_bVisible; }
