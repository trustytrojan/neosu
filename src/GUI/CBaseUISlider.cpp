...
void CBaseUISlider::draw(Graphics *g) {
    ...
    if(m_bDrawFrame) g->drawRect(m_vPos.x, m_vPos.y, m_vSize.x, m_vSize.y + 1);

    // draw sliding line
    if(!m_bHorizontal)
        g->drawLine(m_vPos.x + m_vSize.x / 2.0f, m_vPos.y + m_vBlockSize.y / 2.0, m_vPos.x + m_vSize.x / 2.0f,
                    m_vPos.y + m_vSize.y - m_vBlockSize.y / 2.0f);
    else
        g->drawLine(m_vPos.x + (m_vBlockSize.x - 1) / 2 + 1, m_vPos.y + m_vSize.y / 2.0f + 1,
                    m_vPos.x + m_vSize.x - (m_vBlockSize.x - 1) / 2, m_vPos.y + m_vSize.y / 2.0f + 1);
...
void CBaseUISlider::fireChangeCallback() {
    if(m_sliderChangeCallback != NULL) m_sliderChangeCallback(this);
}
...
CBaseUISlider *CBaseUISlider::setInitialValue(float value) {
    ...
    if(m_fCurValue == m_fMaxValue) percent = 1.0f;
...
void CBaseUISlider::setBlockSize(float xSize, float ySize) { m_vBlockSize = Vector2(xSize, ySize); }
...
bool CBaseUISlider::hasChanged() {
    if(anim->isAnimating(&m_vBlockPos.x)) return true;
    if(m_bHasChanged) {
        m_bHasChanged = false;
        return true;
    }
...
void CBaseUISlider::onFocusStolen() { m_bBusy = false; }

void CBaseUISlider::onMouseUpInside() {
    m_bBusy = false;

    if(m_fCurValue != m_fPrevValue && m_sliderChangeCallback != NULL) m_sliderChangeCallback(this);
}
...
void CBaseUISlider::onMouseDownInside() {
    ...
    if(McRect(m_vPos.x + m_vBlockPos.x, m_vPos.y + m_vBlockPos.y, m_vBlockSize.x, m_vBlockSize.y)
...
void CBaseUISlider::onResized() { setValue(getFloat(), false); }
