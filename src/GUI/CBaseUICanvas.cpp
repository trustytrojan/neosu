...
CBaseUICanvas::CBaseUICanvas(float xPos, float yPos, float xSize, float ySize, UString name)
...
void CBaseUICanvas::updateLayout() {
    ...
    for(size_t i = 0; i < m_vElements.size(); i++) {
        if(!m_vElements[i]->isScaledByHeightOnly())
            ...
            m_vElements[i]->setSizeAbsolute(m_vElements[i]->getRelSize().x * ((m_vSize.y / 9) * 16),
                                            m_vElements[i]->getRelSize().y * m_vSize.y);
...
void CBaseUICanvas::onResized() {
    for(size_t i = 0; i < m_vElements.size(); i++) {
        if(!m_vElements[i]->isScaledByHeightOnly())
            ...
            m_vElements[i]->setSizeAbsolute(m_vElements[i]->getRelSize().x * ((m_vSize.y / 9) * 16),
                                            m_vElements[i]->getRelPos().y * m_vSize.y);
