...
CBaseUIContainerVerticalBox::CBaseUIContainerVerticalBox(float posX, float posY, float sizeX, float sizeY, UString name)
...
void CBaseUIContainerVerticalBox::updateLayout() {
    ...
    for(size_t i = 0; i < m_vElements.size(); i++) {
        if(!m_bHeightOverride && !m_bScaleByHeightOnly) {
            ...
            m_vElements[i]->setPosAbsolute(m_vPos.x + m_vMargin.x,
                                           m_vPos.y + m_vMargin.y + (m_padding + heightStorage) * i);
        }

        else {
            if(!m_bScaleByHeightOnly)
                m_vElements[i]->setSizeAbsolute(m_vSize.x - 2 * m_vMargin.x,
                                                m_vSize.y * m_vElements[i]->getRelSize().y);
            else
                m_vElements[i]->setSizeAbsolute(m_vSize.x - 2 * m_vMargin.x,
                                                m_vSize.x / 16 * 9 * m_vElements[i]->getRelSize().y);

            m_vElements[i]->setPosAbsolute(m_vPos.x + m_vMargin.x,
                                           posStorage + (posStorage == m_vPos.y ? m_padding : m_vMargin.y));
...
void CBaseUIContainerVerticalBox::updateElement(CBaseUIElement *element) { updateLayout(); }
