...
inline Matrix2::Matrix2(const float src[4]) { this->set(src); }

inline Matrix2::Matrix2(float m0, float m1, float m2, float m3) { this->set(m0, m1, m2, m3); }
...
inline void Matrix2::setColumn(int index, const float col[2]) {
    this->m[index * 2] = col[0];
    this->m[index * 2 + 1] = col[1];
}

inline void Matrix2::setColumn(int index, const Vector2& v) {
    this->m[index * 2] = v.x;
    this->m[index * 2 + 1] = v.y;
}

inline const float* Matrix2::get() const { return this->m; }

inline Matrix2& Matrix2::identity() {
    this->m[0] = this->m[3] = 1.0f;
    this->m[1] = this->m[2] = 0.0f;
    return *this;
}

inline Matrix2 Matrix2::operator+(const Matrix2& rhs) const {
    return Matrix2(this->m[0] + rhs[0], this->m[1] + rhs[1], this->m[2] + rhs[2], this->m[3] + rhs[3]);
}
...
inline Vector2 Matrix2::operator*(const Vector2& rhs) const {
    return Vector2(this->m[0] * rhs.x + this->m[2] * rhs.y, this->m[1] * rhs.x + this->m[3] * rhs.y);
}

inline Matrix2 Matrix2::operator*(const Matrix2& rhs) const {
    return Matrix2(this->m[0] * rhs[0] + this->m[2] * rhs[1], this->m[1] * rhs[0] + this->m[3] * rhs[1],
                   this->m[0] * rhs[2] + this->m[2] * rhs[3], this->m[1] * rhs[2] + this->m[3] * rhs[3]);
}
...
inline float Matrix2::operator[](int index) const { return this->m[index]; }

inline float& Matrix2::operator[](int index) { return this->m[index]; }
...
inline Matrix3::Matrix3(const float src[9]) { this->set(src); }
...
inline void Matrix3::setRow(int index, const float row[3]) {
    this->m[index] = row[0];
    this->m[index + 3] = row[1];
    this->m[index + 6] = row[2];
}
...
inline void Matrix3::setColumn(int index, const float col[3]) {
    this->m[index * 3] = col[0];
    this->m[index * 3 + 1] = col[1];
    this->m[index * 3 + 2] = col[2];
}

inline void Matrix3::setColumn(int index, const Vector3& v) {
    this->m[index * 3] = v.x;
    this->m[index * 3 + 1] = v.y;
    this->m[index * 3 + 2] = v.z;
}

inline const float* Matrix3::get() const { return this->m; }

inline Matrix3& Matrix3::identity() {
    this->m[0] = this->m[4] = this->m[8] = 1.0f;
    this->m[1] = this->m[2] = this->m[3] = this->m[5] = this->m[6] = this->m[7] = 0.0f;
    return *this;
}

inline Matrix3 Matrix3::operator+(const Matrix3& rhs) const {
    return Matrix3(this->m[0] + rhs[0], this->m[1] + rhs[1], this->m[2] + rhs[2], this->m[3] + rhs[3],
                   this->m[4] + rhs[4], this->m[5] + rhs[5], this->m[6] + rhs[6], this->m[7] + rhs[7],
                   this->m[8] + rhs[8]);
}
...
inline Vector3 Matrix3::operator*(const Vector3& rhs) const {
    return Vector3(this->m[0] * rhs.x + this->m[3] * rhs.y + this->m[6] * rhs.z,
                   this->m[1] * rhs.x + this->m[4] * rhs.y + this->m[7] * rhs.z,
                   this->m[2] * rhs.x + this->m[5] * rhs.y + this->m[8] * rhs.z);
}

inline Matrix3 Matrix3::operator*(const Matrix3& rhs) const {
    return Matrix3(this->m[0] * rhs[0] + this->m[3] * rhs[1] + this->m[6] * rhs[2],
                   this->m[1] * rhs[0] + this->m[4] * rhs[1] + this->m[7] * rhs[2],
                   this->m[2] * rhs[0] + this->m[5] * rhs[1] + this->m[8] * rhs[2],
                   this->m[0] * rhs[3] + this->m[3] * rhs[4] + this->m[6] * rhs[5],
                   this->m[1] * rhs[3] + this->m[4] * rhs[4] + this->m[7] * rhs[5],
                   this->m[2] * rhs[3] + this->m[5] * rhs[4] + this->m[8] * rhs[5],
                   this->m[0] * rhs[6] + this->m[3] * rhs[7] + this->m[6] * rhs[8],
                   this->m[1] * rhs[6] + this->m[4] * rhs[7] + this->m[7] * rhs[8],
                   this->m[2] * rhs[6] + this->m[5] * rhs[7] + this->m[8] * rhs[8]);
}
...
inline float Matrix3::operator[](int index) const { return this->m[index]; }

inline float& Matrix3::operator[](int index) { return this->m[index]; }
...
inline Matrix4::Matrix4(const float src[16]) { this->set(src); }
...
inline void Matrix4::setRow(int index, const float row[4]) {
    this->m[index] = row[0];
    this->m[index + 4] = row[1];
    this->m[index + 8] = row[2];
    this->m[index + 12] = row[3];
}
...
inline void Matrix4::setColumn(int index, const float col[4]) {
    this->m[index * 4] = col[0];
    this->m[index * 4 + 1] = col[1];
    this->m[index * 4 + 2] = col[2];
    this->m[index * 4 + 3] = col[3];
}

inline void Matrix4::setColumn(int index, const Vector4& v) {
    this->m[index * 4] = v.x;
    this->m[index * 4 + 1] = v.y;
    this->m[index * 4 + 2] = v.z;
    this->m[index * 4 + 3] = v.w;
}
...
inline const float* Matrix4::get() const { return this->m; }
...
inline const float* Matrix4::getTranspose() {
    ...
    this->tm[8] = this->m[2];
    this->tm[9] = this->m[6];
    this->tm[10] = this->m[10];
    this->tm[11] = this->m[14];
    this->tm[12] = this->m[3];
    this->tm[13] = this->m[7];
    this->tm[14] = this->m[11];
    this->tm[15] = this->m[15];
    return this->tm;
}

inline Matrix4& Matrix4::identity() {
    this->m[0] = this->m[5] = this->m[10] = this->m[15] = 1.0f;
    this->m[1] = this->m[2] = this->m[3] = this->m[4] = this->m[6] = this->m[7] = this->m[8] = this->m[9] =
        this->m[11] = this->m[12] = this->m[13] = this->m[14] = 0.0f;
    return *this;
}

inline Matrix4 Matrix4::operator+(const Matrix4& rhs) const {
    return Matrix4(this->m[0] + rhs[0], this->m[1] + rhs[1], this->m[2] + rhs[2], this->m[3] + rhs[3],
                   this->m[4] + rhs[4], this->m[5] + rhs[5], this->m[6] + rhs[6], this->m[7] + rhs[7],
                   this->m[8] + rhs[8], this->m[9] + rhs[9], this->m[10] + rhs[10], this->m[11] + rhs[11],
                   this->m[12] + rhs[12], this->m[13] + rhs[13], this->m[14] + rhs[14], this->m[15] + rhs[15]);
}
...
inline Vector4 Matrix4::operator*(const Vector4& rhs) const {
    return Vector4(this->m[0] * rhs.x + this->m[4] * rhs.y + this->m[8] * rhs.z + this->m[12] * rhs.w,
                   this->m[1] * rhs.x + this->m[5] * rhs.y + this->m[9] * rhs.z + this->m[13] * rhs.w,
                   this->m[2] * rhs.x + this->m[6] * rhs.y + this->m[10] * rhs.z + this->m[14] * rhs.w,
                   this->m[3] * rhs.x + this->m[7] * rhs.y + this->m[11] * rhs.z + this->m[15] * rhs.w);
}

inline Vector3 Matrix4::operator*(const Vector3& rhs) const {
    return Vector3(this->m[0] * rhs.x + this->m[4] * rhs.y + this->m[8] * rhs.z,
                   this->m[1] * rhs.x + this->m[5] * rhs.y + this->m[9] * rhs.z,
                   this->m[2] * rhs.x + this->m[6] * rhs.y + this->m[10] * rhs.z);
}

inline Matrix4 Matrix4::operator*(const Matrix4& n) const {
    return Matrix4(this->m[0] * n[0] + this->m[4] * n[1] + this->m[8] * n[2] + this->m[12] * n[3],
                   this->m[1] * n[0] + this->m[5] * n[1] + this->m[9] * n[2] + this->m[13] * n[3],
                   this->m[2] * n[0] + this->m[6] * n[1] + this->m[10] * n[2] + this->m[14] * n[3],
                   this->m[3] * n[0] + this->m[7] * n[1] + this->m[11] * n[2] + this->m[15] * n[3],
                   this->m[0] * n[4] + this->m[4] * n[5] + this->m[8] * n[6] + this->m[12] * n[7],
                   this->m[1] * n[4] + this->m[5] * n[5] + this->m[9] * n[6] + this->m[13] * n[7],
                   this->m[2] * n[4] + this->m[6] * n[5] + this->m[10] * n[6] + this->m[14] * n[7],
                   this->m[3] * n[4] + this->m[7] * n[5] + this->m[11] * n[6] + this->m[15] * n[7],
                   this->m[0] * n[8] + this->m[4] * n[9] + this->m[8] * n[10] + this->m[12] * n[11],
                   this->m[1] * n[8] + this->m[5] * n[9] + this->m[9] * n[10] + this->m[13] * n[11],
                   this->m[2] * n[8] + this->m[6] * n[9] + this->m[10] * n[10] + this->m[14] * n[11],
                   this->m[3] * n[8] + this->m[7] * n[9] + this->m[11] * n[10] + this->m[15] * n[11],
                   this->m[0] * n[12] + this->m[4] * n[13] + this->m[8] * n[14] + this->m[12] * n[15],
                   this->m[1] * n[12] + this->m[5] * n[13] + this->m[9] * n[14] + this->m[13] * n[15],
                   this->m[2] * n[12] + this->m[6] * n[13] + this->m[10] * n[14] + this->m[14] * n[15],
                   this->m[3] * n[12] + this->m[7] * n[13] + this->m[11] * n[14] + this->m[15] * n[15]);
}
...
inline bool Matrix4::operator==(const Matrix4& n) const {
    return (this->m[0] == n[0]) && (this->m[1] == n[1]) && (this->m[2] == n[2]) && (this->m[3] == n[3]) &&
           (this->m[4] == n[4]) && (this->m[5] == n[5]) && (this->m[6] == n[6]) && (this->m[7] == n[7]) &&
           (this->m[8] == n[8]) && (this->m[9] == n[9]) && (this->m[10] == n[10]) && (this->m[11] == n[11]) &&
           (this->m[12] == n[12]) && (this->m[13] == n[13]) && (this->m[14] == n[14]) && (this->m[15] == n[15]);
}
...
inline float Matrix4::operator[](int index) const { return this->m[index]; }

inline float& Matrix4::operator[](int index) { return this->m[index]; }
...
inline Matrix4 operator*(float s, const Matrix4& rhs) {
    return Matrix4(s * rhs[0], s * rhs[1], s * rhs[2], s * rhs[3], s * rhs[4], s * rhs[5], s * rhs[6], s * rhs[7],
                   s * rhs[8], s * rhs[9], s * rhs[10], s * rhs[11], s * rhs[12], s * rhs[13], s * rhs[14],
                   s * rhs[15]);
}
