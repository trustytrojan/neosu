   ...
   public:
    ...
    Matrix3(const float src[9]) {
        for(int col = 0; col < 3; ++col)
            for(int row = 0; row < 3; ++row) m_mat[col][row] = src[col * 3 + row];
    }
   ...
   public:
    ...
    Matrix4(const float src[16]) {
        for(int col = 0; col < 4; ++col)
            for(int row = 0; row < 4; ++row) m_mat[col][row] = src[col * 4 + row];
    }
    Matrix4(float m00, float m01, float m02, float m03, float m04, float m05, float m06, float m07, float m08,
