...
void *asyncResourceLoaderThread(void *data, std::stop_token stopToken) {
    ...
    if(cv_debug_rm.getBool()) debugLog("AsyncResourceLoader: Thread #{} started\n", threadIndex);
    ...
    if(cv_debug_rm.getBool()) debugLog("AsyncResourceLoader: Thread #{} exiting\n", threadIndex);
...
void AsyncResourceLoader::update(bool lowLatency) {
    ...
    while(numProcessed < amountToProcess) {
        ...
        if(cv_debug_rm.getBool()) debugLog("AsyncResourceLoader: Sync init for {:s}\n", rs->getName());
    ...
    for(Resource *rs : resourcesReadyForDestroy) {
        if(cv_debug_rm.getBool()) debugLog("AsyncResourceLoader: Async destroy of resource {:s}\n", rs->getName());
...
void AsyncResourceLoader::scheduleAsyncDestroy(Resource *resource) {
    if(cv_debug_rm.getBool()) debugLog("AsyncResourceLoader: Scheduled async destroy of {:s}\n", resource->getName());
...
void AsyncResourceLoader::reloadResources(const std::vector<Resource *> &resources) {
    ...
    if(cv_debug_rm.getBool()) debugLog("AsyncResourceLoader: Async reloading {} resources\n", resources.size());

    std::vector<Resource *> resourcesToReload;
    for(Resource *rs : resources) {
        if(rs == nullptr) continue;

        if(cv_debug_rm.getBool()) debugLog("AsyncResourceLoader: Async reloading {:s}\n", rs->getName());
...
void AsyncResourceLoader::ensureThreadAvailable() {
    ...
    if(activeWorkCount > activeThreads && activeThreads < this->iMaxThreads) {
        ...
        if(this->threads.size() < this->iMaxThreads) {
            ...
            loaderThread->thread = std::make_unique<McThread>(asyncResourceLoaderThread, loaderThread.get());
            if(!loaderThread->thread->isReady()) {
                if(cv_debug_rm.getBool()) debugLog("AsyncResourceLoader Warning: Couldn't create dynamic thread!\n");
            } else {
                if(cv_debug_rm.getBool())
