...
/// ConVar snd_async("snd_async", true, FCVAR_NONE, "play sounds on separate thread (e.g. removes BASS_ChannelPlay()
/// ~0.3 ms blocking delay from main thread)");
...
ConVar win_snd_wasapi_exclusive("win_snd_wasapi_exclusive", true, FCVAR_NONE,
                                "whether to use exclusive device mode to further reduce latency",
                                _WIN_SND_WASAPI_EXCLUSIVE_CHANGE);
ConVar win_snd_wasapi_shared_volume_affects_device(
    "win_snd_wasapi_shared_volume_affects_device", false, FCVAR_NONE,
...
SoundEngine::SoundEngine() {
    ...
    // BASS_SetConfig(BASS_CONFIG_DEV_BUFFER, 10); // NOTE: only used by new osu atm, but not tested enough for offset
    // problems
    BASS_SetConfig(BASS_CONFIG_MP3_OLDGAPS, 1);  // NOTE: only used by osu atm (all beatmaps timed to non-iTunesSMPB +
                                                 // 529 sample deletion offsets on old dlls pre 2015)
    BASS_SetConfig(BASS_CONFIG_DEV_NONSTOP,
                   1);  // NOTE: only used by osu atm (avoids lag/jitter in BASS_ChannelGetPosition() shortly after a
                        // BASS_ChannelPlay() after loading/silence)
    ...
    BASS_SetConfig(
        ...
        10);  // NOTE: only used by osu atm (new osu uses 5 instead of 10, but not tested enough for offset problems)
...
void SoundEngine::updateOutputDevices(bool printInfo) {
    ...
    for(int d = 0; (BASS_GetDeviceInfo(d, &deviceInfo) == true); d++) {
        ...
        if(d > 0 || allowNoSoundDevice)  // the first device doesn't count ("No sound") ~ Default in array
        {
            ...
            // TODO: this causes endless loops on audio devices with chinese/russian etc. characters in their name, may
            // be caused by BASS reordering indices
            /*
            if (handleOutputDeviceChanges)
            {
                    if (d == currentOutputDeviceBASSIndex && m_outputDevices.size() > d)
                    {
                            ...
                            if (hasChanged)
                            {
                                    debugLog("SoundEngine: Output device [\"%s\", enabled = %i, default = %i] changed to
            [\"%s\", enabled = %i, default = %i]\n", m_outputDevices[d].name.toUtf8(), (int)m_outputDevices[d].enabled,
            (int)m_outputDevices[d].isDefault, soundDevice.name.toUtf8(), (int)soundDevice.enabled,
            (int)soundDevice.isDefault);
                                    ...
                                    // special case: if the user is using "Default" (index -1) and the actual system
            default changed, then we automatically re-initialize the device if (m_iCurrentOutputDevice == -1 &&
            !outputDeviceChange)
                                    {
...
bool SoundEngine::initializeOutputDevice(int id) {
    debugLog("SoundEngine: initializeOutputDevice( %i, fallback = %i ) ...\n", id,
             (int)win_snd_fallback_dsound.getBool());
    ...
    ret = BASS_WASAPI_Init(id, 0, 0, (win_snd_wasapi_exclusive.getBool() ? BASS_WASAPI_EXCLUSIVE : 0), bufferSize,
                           updatePeriod, OutputWasapiProc, NULL);
...
SoundEngine::~SoundEngine() {
    ...
    if(m_bReady) {
        ...
        // NOTE: commented, because for some reason this now crashes in dllunload on Windows 10? this cleanup is
        // irrelevant anyway since it only happens on shutdown
        // BASS_WASAPI_Free();
...
void SoundEngine::restart() { setOutputDeviceForce(m_sCurrentOutputDevice); }
...
bool SoundEngine::play(Sound *snd, float pan, float pitch) {
    ...
    if(snd->isStream() && snd->isLooped()) BASS_ChannelFlags(handle, BASS_SAMPLE_LOOP, BASS_SAMPLE_LOOP);

    if(!snd->isStream() && LOWORD(m_iBASSVersion) >= 0x0c00)  // BASS_ATTRIB_NORAMP is available >= 2.4.12 - 10/3/2016
        BASS_ChannelSetAttribute(handle, BASS_ATTRIB_NORAMP,
                                 1.0f);  // see https://github.com/ppy/osu-framework/pull/3146
    ...
    if(BASS_ChannelIsActive(handle) != BASS_ACTIVE_PLAYING) {
        ...
        if(!ret) debugLog("SoundEngine::play() couldn't BASS_ChannelPlay(), errorcode %i\n", BASS_ErrorGetCode());
    ...
    if(BASS_ChannelIsActive(handle) != BASS_ACTIVE_PLAYING) {
        ...
        if(!snd->isStream() &&
           LOWORD(m_iBASSVersion) >= 0x0c00)  // BASS_ATTRIB_NORAMP is available >= 2.4.12 - 10/3/2016
            BASS_ChannelSetAttribute(handle, BASS_ATTRIB_NORAMP,
                                     1.0f);  // see https://github.com/ppy/osu-framework/pull/3146
        ...
        */
        ...
        if(ret) snd->setLastPlayTime(engine->getTime());
    ...
    if(snd->isStream() && Mix_PlayingMusic() && Mix_PausedMusic()) {
        ...
        if(snd->isLooped()) return false;

        int channel = (snd->isStream() ? (Mix_PlayMusic((Mix_Music *)snd->getMixChunkOrMixMusic(), 1))
                                       : Mix_PlayChannel(-1, (Mix_Chunk *)snd->getMixChunkOrMixMusic(), 0));
...
void SoundEngine::pause(Sound *snd) {
    ...
    if(snd->isStream())
        Mix_PauseMusic();
    else if(!snd->isLooped())  // special case: looped sounds are not supported for sdl/mixer, so do not let them kill
                               // other channels
...
void SoundEngine::stop(Sound *snd) {
    ...
    if(BASS_Mixer_ChannelGetMixer(handle) != 0) BASS_Mixer_ChannelRemove(handle);
    ...
    if(snd->isStream())
        Mix_HaltMusic();
    else if(!snd->isLooped())  // special case: looped sounds are not supported here, so do not let them kill other
                               // channels
        ...
        if(m_outputDevices[i].name == outputDeviceName) {
            if(m_outputDevices[i].id != m_iCurrentOutputDevice) {
                ...
                if(!initializeOutputDevice(m_outputDevices[i].id))
                    initializeOutputDevice(previousOutputDevice);  // if something went wrong, automatically switch back
...
void SoundEngine::setVolume(float volume) {
    ...
    BASS_WASAPI_SetVolume(BASS_WASAPI_CURVE_WINDOWS | (!win_snd_wasapi_exclusive.getBool() &&
                                                               !win_snd_wasapi_shared_volume_affects_device.getBool()
                                                           ? BASS_WASAPI_VOL_SESSION
                                                           : 0),
                          m_fVolume);
