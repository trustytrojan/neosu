...
void DirectX11Image::init() {
    if((m_texture != NULL && !m_bKeepInSystemMemory) || !m_bAsyncReady)
        return;  // only load if we are not already loaded

    HRESULT hr;

    DirectX11Interface *g = ((DirectX11Interface *)engine->getGraphics());
    if(m_interfaceOverrideHack != NULL) g = m_interfaceOverrideHack;
    ...
    D3D11_TEXTURE2D_DESC textureDesc;
    {
        ...
        textureDesc.Format =
            m_iNumChannels == 4
                ...
                : (m_iNumChannels == 3 ? DXGI_FORMAT::DXGI_FORMAT_R8_UNORM
                                       : (m_iNumChannels == 1 ? DXGI_FORMAT::DXGI_FORMAT_R8_UNORM
                                                              : DXGI_FORMAT::DXGI_FORMAT_R8G8B8A8_UNORM));
        textureDesc.SampleDesc.Count = 1;
        textureDesc.SampleDesc.Quality = 0;
        textureDesc.Usage =
            (m_bKeepInSystemMemory ? D3D11_USAGE::D3D11_USAGE_DYNAMIC : D3D11_USAGE::D3D11_USAGE_DEFAULT);
    ...
    if(m_texture == NULL) {
        ...
        hr = g->getDevice()->CreateTexture2D(
            &textureDesc, (m_rawImage.size() >= m_iWidth * m_iHeight * m_iNumChannels ? &initData : NULL), &m_texture);
        if(FAILED(hr) || m_texture == NULL) {
            debugLog("DirectX Image Error: Couldn't CreateTexture2D(%ld, %x, %x) on file %s!\n", hr, hr,
                     MAKE_DXGI_HRESULT(hr), m_sFilePath.c_str());
            engine->showMessageError(
                "Image Error", UString::format("DirectX Image error, couldn't CreateTexture2D(%ld, %x, %x) on file %s",
        ...
        if(FAILED(hr) || m_shaderResourceView == NULL) {
            ...
            engine->showMessageError(
                ...
                UString::format("DirectX Image error, couldn't CreateShaderResourceView(%ld, %x, %x) on file %s", hr,
                                hr, MAKE_DXGI_HRESULT(hr), m_sFilePath.c_str()));
    ...
    if(m_samplerState == NULL) {
        ...
        engine->showMessageError("Image Error",
                                 UString::format("Couldn't CreateSamplerState() on file %s!", m_sFilePath.c_str()));
        return;
    }
...
void DirectX11Image::bind(unsigned int textureUnit) {
    ...
    ((DirectX11Interface *)engine->getGraphics())
        ->getDeviceContext()
        ->PSGetShaderResources(textureUnit, 1, &m_prevShaderResourceView);  // backup

    ((DirectX11Interface *)engine->getGraphics())
        ->getDeviceContext()
        ->PSSetShaderResources(textureUnit, 1, &m_shaderResourceView);
...
void DirectX11Image::unbind() {
    ...
    ((DirectX11Interface *)engine->getGraphics())
        ->getDeviceContext()
        ->PSSetShaderResources(m_iTextureUnitBackup, 1, &m_prevShaderResourceView);  // restore
...
void DirectX11Image::setFilterMode(Graphics::FILTER_MODE filterMode) {
    ...
    switch(filterMode) {
        ...
        case Graphics::FILTER_MODE::FILTER_MODE_MIPMAP:
            // TODO: the name "FILTER_MODE_MIPMAP" is actually bad, should probably be FILTER_MODE_ANISOTROPIC? (i.e.
            // GL_LINEAR_MIPMAP_LINEAR min should be default)
            m_samplerDesc.Filter = D3D11_FILTER::D3D11_FILTER_MIN_MAG_MIP_LINEAR;
            break;
    }
