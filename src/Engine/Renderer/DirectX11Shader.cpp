...
void DirectX11Shader::init() { m_bReady = compile(m_sVsh, m_sFsh, m_bSource); }

void DirectX11Shader::initAsync() { m_bAsyncReady = true; }

void DirectX11Shader::destroy() {
    if(m_constantBuffer != NULL) m_constantBuffer->Release();

    if(m_inputLayout != NULL) m_inputLayout->Release();

    if(m_vs != NULL) m_vs->Release();
...
void DirectX11Shader::setUniform1f(UString name, float value) {
    ...
    if(FAILED(dx11->getDeviceContext()->Map(m_constantBuffer, 0, D3D11_MAP::D3D11_MAP_WRITE_DISCARD, 0,
                                            &mappedResource))) {
        if(debug_shaders->getBool()) debugLog("DirectX11Shader::setUniformMatrix4fv() couldn't Map()!\n");
...
bool DirectX11Shader::compile(std::string vertexShader, std::string fragmentShader, bool source) {
    ...
    const D3D_SHADER_MACRO defines[] = {
        "EXAMPLE_DEFINE", "1", NULL, NULL  // sentinel
    };
    ...
    hr1 = D3DCompile(vertexShader.c_str(), vertexShader.length(), "VS", defines, NULL, vsEntryPoint, vsProfile, flags,
                     0, &vs, &vsError);

    debugLog("DirectX11Shader: Compiling %s ...\n", (source ? "pixel source" : fragmentShader.c_str()));
    hr2 = D3DCompile(fragmentShader.c_str(), fragmentShader.length(), "PS", defines, NULL, psEntryPoint, psProfile,
                     flags, 0, &ps, &psError);
    ...
    D3D11_INPUT_ELEMENT_DESC elements[] = {
        {"POSITION", 0, DXGI_FORMAT::DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
         D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA, 0},  // + 3*4
        {"COLOR", 0, DXGI_FORMAT::DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 12,
         D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA, 0},  // + 4*4
        {"TEXCOORD", 0, DXGI_FORMAT::DXGI_FORMAT_R32G32_FLOAT, 0, 28,
         D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA, 0},  // + 2*4
        //{ "NORMAL",	0, DXGI_FORMAT_R32G32B32A32_FLOAT,				0,	36,
        // D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA, 0 },	// + 3*4
    };
    UINT numElements = _countof(elements);

    hr1 = dx11->getDevice()->CreateInputLayout(elements, numElements, vs->GetBufferPointer(), vs->GetBufferSize(),
                                               &m_inputLayout);
    ...
    D3D11_BUFFER_DESC matrixBufferDesc;
    {
        ...
        matrixBufferDesc.ByteWidth =
            sizeof(MatrixBufferType);  // NOTE: "If the bind flag is D3D11_BIND_CONSTANT_BUFFER, you must set the
                                       // ByteWidth value in multiples of 16, and less than or equal to
    ...
    if(FAILED(hr1)) {
        engine->showMessageError("DirectX11Shader Error", UString::format("Couldn't CreateBuffer(%ld, %x, %x)!", hr1,
                                                                          hr1, MAKE_DXGI_HRESULT(hr1)));
        return false;
    }
