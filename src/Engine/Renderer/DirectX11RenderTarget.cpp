...
void DirectX11RenderTarget::initAsync() { m_bAsyncReady = true; }
...
void DirectX11RenderTarget::enable() {
    ...
    ((DirectX11Interface *)engine->getGraphics())
        ->getDeviceContext()
        ->OMGetRenderTargets(1, &m_prevRenderTargetView, &m_prevDepthStencilView);  // backup
    ...
    if(debug_rt->getBool()) clearColor = COLORf(0.5f, 0.0f, 0.5f, 0.0f);

    float fClearColor[4] = {COLOR_GET_Rf(clearColor), COLOR_GET_Gf(clearColor), COLOR_GET_Bf(clearColor),
                            COLOR_GET_Af(clearColor)};

    if(m_bClearColorOnDraw)
        ((DirectX11Interface *)engine->getGraphics())
            ->getDeviceContext()
            ->ClearRenderTargetView(m_renderTargetView, fClearColor);

    if(m_bClearDepthOnDraw)
        ((DirectX11Interface *)engine->getGraphics())
            ->getDeviceContext()
            ->ClearDepthStencilView(m_depthStencilView, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f,
...
void DirectX11RenderTarget::disable() {
    ...
    ((DirectX11Interface *)engine->getGraphics())
        ->getDeviceContext()
        ->OMSetRenderTargets(1, &m_prevRenderTargetView, m_prevDepthStencilView);  // restore
