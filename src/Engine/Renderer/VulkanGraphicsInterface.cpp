...
class VulkanSwapChain {
   public:
    VulkanSwapChain(VulkanGraphicsInterface *graphics) { m_graphics = graphics; }
...
VulkanGraphicsInterface::VulkanGraphicsInterface() : NullGraphicsInterface() {
    if(!vulkan->isReady()) {
        engine->showMessageErrorFatal(
            "Fatal Vulkan Error", "Couldn't initialize VulkanGraphicsInterface because VulkanInterface is not ready!");
        exit(0);
        return;
    }
...
#error "TODO: add correct logic here"
    ...
    if(res != VK_SUCCESS) {
        engine->showMessageErrorFatal("Fatal Vulkan Error",
                                      UString::format("Couldn't vkCreate*SurfaceKHR(), returned %d", res));
        exit(0);
        return;
    }
}
...
VulkanGraphicsInterface::~VulkanGraphicsInterface() {
    if(m_surface != 0) vkDestroySurfaceKHR(vulkan->getInstance(), m_surface, NULL);
...
void VulkanGraphicsInterface::drawPixels(int x, int y, int width, int height, Graphics::DRAWPIXELS_TYPE type,
                                         const void *pixels) {}
...
void VulkanGraphicsInterface::drawRect(int x, int y, int width, int height, Color top, Color right, Color bottom,
                                       Color left) {}
...
void VulkanGraphicsInterface::fillGradient(int x, int y, int width, int height, Color topLeftColor, Color topRightColor,
                                           Color bottomLeftColor, Color bottomRightColor) {}

void VulkanGraphicsInterface::drawQuad(int x, int y, int width, int height) {}

void VulkanGraphicsInterface::drawQuad(Vector2 topLeft, Vector2 topRight, Vector2 bottomRight, Vector2 bottomLeft,
...
std::vector<unsigned char> VulkanGraphicsInterface::getScreenshot() { return std::vector<unsigned char>(); }

UString VulkanGraphicsInterface::getVendor() { return "<VENDOR>"; }

UString VulkanGraphicsInterface::getModel() { return "<MODEL>"; }

UString VulkanGraphicsInterface::getVersion() { return "<VERSION>"; }

int VulkanGraphicsInterface::getVRAMTotal() { return -1; }

int VulkanGraphicsInterface::getVRAMRemaining() { return -1; }

void VulkanGraphicsInterface::onResolutionChange(Vector2 newResolution) { m_vResolution = newResolution; }
...
RenderTarget *VulkanGraphicsInterface::createRenderTarget(int x, int y, int width, int height,
                                                          Graphics::MULTISAMPLE_TYPE multiSampleType) {
    return new NullRenderTarget(x, y, width, height, multiSampleType);
}

Shader *VulkanGraphicsInterface::createShaderFromFile(std::string vertexShaderFilePath,
...
VertexArrayObject *VulkanGraphicsInterface::createVertexArrayObject(Graphics::PRIMITIVE primitive,
