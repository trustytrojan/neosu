...
DirectX11Interface::~DirectX11Interface() {
    if(!m_bMinimalistContext) {
        if(m_swapChain != NULL) m_swapChain->SetFullscreenState(FALSE, NULL);
        ...
        if(m_swapChain != NULL) m_swapChain->Release();

        if(m_frameBuffer != NULL) m_frameBuffer->Release();

        if(m_frameBufferDepthStencilView != NULL) m_frameBufferDepthStencilView->Release();

        if(m_frameBufferDepthStencilTexture != NULL) m_frameBufferDepthStencilTexture->Release();
    }

    if(m_device != NULL) m_device->Release();

    if(m_deviceContext != NULL) m_deviceContext->Release();
}
...
void DirectX11Interface::init() {
    ...
    ZeroMemory(&m_swapChainModeDesc, sizeof(DXGI_MODE_DESC));
    {
        ...
        m_swapChainModeDesc.Format = DXGI_FORMAT::DXGI_FORMAT_R8G8B8A8_UNORM;  // NOTE: DXGI_FORMAT_R8G8B8A8_UNORM has
                                                                               // the broadest compatibility range
    ...
    ZeroMemory(&swapChainDesc, sizeof(DXGI_SWAP_CHAIN_DESC));
    {
        ...
        swapChainDesc.SwapEffect =
            DXGI_SWAP_EFFECT::DXGI_SWAP_EFFECT_DISCARD;  // TODO: Windows 10 and newer support flip to avoid the blit,
    ...
    if(m_bMinimalistContext)
        hr = D3D11CreateDevice(NULL, D3D_DRIVER_TYPE::D3D_DRIVER_TYPE_HARDWARE, NULL, createDeviceFlags, featureLevels,
                               numFeatureLevels, D3D11_SDK_VERSION, &m_device, &featureLevelOut, &m_deviceContext);
    else
        hr = D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE::D3D_DRIVER_TYPE_HARDWARE, NULL, createDeviceFlags,
                                           featureLevels, numFeatureLevels, D3D11_SDK_VERSION, &swapChainDesc,
                                           &m_swapChain, &m_device, &featureLevelOut, &m_deviceContext);
    ...
    // disable hardcoded DirectX ALT + ENTER fullscreen toggle functionality (this is instead handled by the engine
    // internally) disable dxgi interfering with mode changes and WndProc (again, handled by the engine internally)
    {
    ...
    std::string vertexShader =
        ...
        //"	Out.pos.z = (Out.pos.z + Out.pos.w)/2.0f;\n" // TODO: not sure if necessary to compensate clip space
        // range here, no artifacts so far (OpenGL NDC z from -1 to 1, DirectX NDC z from 0 to 1)
        "	Out.col = In.col;\n"
        "	Out.misc = misc;\n"
        "	Out.tex = In.tex;\n"
        "	return Out;\n"
...
void DirectX11Interface::beginScene() {
    ...
    if(m_frameBuffer != NULL) m_deviceContext->ClearRenderTargetView(m_frameBuffer, clearColor);
...
void DirectX11Interface::setColor(Color color) {
    ...
    m_color = color;
    m_shaderTexturedGeneric->setUniform4f("col", COLOR_GET_Af(m_color), COLOR_GET_Rf(m_color), COLOR_GET_Gf(m_color),
                                          COLOR_GET_Bf(m_color));
}
...
void DirectX11Interface::drawPixels(int x, int y, int width, int height, Graphics::DRAWPIXELS_TYPE type,
                                    const void *pixels) {
    // TODO: implement
}
...
void DirectX11Interface::drawLine(Vector2 pos1, Vector2 pos2) { drawLine(pos1.x, pos1.y, pos2.x, pos2.y); }
...
void DirectX11Interface::drawRect(int x, int y, int width, int height, Color top, Color right, Color bottom,
                                  Color left) {
    setColor(top);
    drawLine(x, y, x + width, y);
    setColor(left);
...
void DirectX11Interface::drawQuad(Vector2 topLeft, Vector2 topRight, Vector2 bottomRight, Vector2 bottomLeft,
...
void DirectX11Interface::drawVAO(VertexArrayObject *vao) {
    ...
    for(size_t i = 0; i < vcolors.size(); i++) {
        const Vector4 color = Vector4(COLOR_GET_Rf(vcolors[i]), COLOR_GET_Gf(vcolors[i]), COLOR_GET_Bf(vcolors[i]),
    ...
    m_vertices.resize(finalVertices.size());
    {
        ...
        const Vector4 color =
            Vector4(COLOR_GET_Rf(m_color), COLOR_GET_Gf(m_color), COLOR_GET_Bf(m_color), COLOR_GET_Af(m_color));
        ...
        for(size_t i = 0; i < finalVertices.size(); i++) {
            ...
            // TODO: multitexturing
            if(hasTexcoords0) m_vertices[i].tex = finalTexcoords[0][clamp<size_t>(i, 0, maxTexcoords0Index)];
        }
    }
...
int DirectX11Interface::getVRAMTotal() {
    ...
    if(SUCCEEDED(m_swapChain->GetParent(__uuidof(IDXGIFactory1), (void **)&pFactory)) && pFactory != NULL) {
        ...
        if(SUCCEEDED(pFactory->EnumAdapters(0, &adapter)) && adapter != NULL) {
            DXGI_ADAPTER_DESC desc;
            if(SUCCEEDED(adapter->GetDesc(&desc))) {
                // NOTE: this value is affected by 32-bit limits, meaning it will cap out at ~3071 MB (or ~3072 MB
                // depending on rounding), which makes sense since we can't address more video memory in a 32-bit
                // process anyway
...
void DirectX11Interface::onResolutionChange(Vector2 newResolution) {
    ...
    if(!engine->isDrawing())  // HACKHACK: to allow viewport changes for rendertarget rendering OpenGL style
    {
        ...
        // resize
        // NOTE: when in fullscreen mode, use 0 as width/height (because they were set internally by
        // SetFullscreenState()) NOTE: DXGI_FORMAT_UNKNOWN preserves the existing format
        hr = m_swapChain->ResizeBuffers(
            ...
            DXGI_FORMAT::DXGI_FORMAT_UNKNOWN, /*DXGI_SWAP_CHAIN_FLAG::DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH*/ 0);
        if(FAILED(hr))
        ...
        // read actual new width/height of backbuffer
        {
            ...
            // NOTE: force overwrite local resolution (even though it was just set at the start of onResolutionChange()
            // here)
        ...
        if(FAILED(hr)) {
            debugLog(
                "FATAL ERROR: DirectX11Interface::onResolutionChange() couldn't CreateRenderTargetView(%ld, %x, "
                "%x)!!!\n",
                hr, hr, MAKE_DXGI_HRESULT(hr));
            m_frameBuffer = NULL;
            return;
        ...
        // add new depth buffer
        {
            ...
            if(FAILED(hr)) {
                debugLog(
                    "FATAL ERROR: DirectX11Interface::onResolutionChange() couldn't CreateTexture2D(%ld, %x, %x)!!!\n",
                    hr, hr, MAKE_DXGI_HRESULT(hr));
                m_frameBufferDepthStencilTexture = NULL;
            } else {
                ...
                if(FAILED(hr)) {
                    debugLog(
                        "FATAL ERROR: DirectX11Interface::onResolutionChange() couldn't CreateDepthStencilView(%ld, "
                        "%x, %x)!!!\n",
                        hr, hr, MAKE_DXGI_HRESULT(hr));
                    m_frameBufferDepthStencilView = NULL;
                }
...
void DirectX11Interface::resizeTarget(Vector2 newResolution) {
    ...
    m_swapChain->ResizeTarget(&m_swapChainModeDesc);  // NOTE: this will resize the actual window and send WM_SIZE which
                                                      // will in turn call onResolutionChange() here
}
...
void DirectX11Interface::disableFullscreen() {
    ...
    if(!m_bIsFullscreenBorderlessWindowed) m_swapChain->SetFullscreenState((WINBOOL) false, NULL);
