...
SWGraphicsInterface::~SWGraphicsInterface() {
    if(m_backBuffer != NULL) delete[] m_backBuffer;
}
...
void SWGraphicsInterface::setColor(Color color) { m_color = color; }
...
void SWGraphicsInterface::drawPixels(int x, int y, int width, int height, Graphics::DRAWPIXELS_TYPE type,
                                     const void *pixels) {
    // TODO:
    /*
...
void SWGraphicsInterface::drawPixel(int x, int y) {
    ...
    Color finalColor = COLORf(
        (COLOR_GET_Af(srcPixel) * COLOR_GET_Af(srcPixel) + COLOR_GET_Af(dstPixel) * (1.0f - COLOR_GET_Af(srcPixel))),
...
void SWGraphicsInterface::drawLine(Vector2 pos1, Vector2 pos2) { drawLine(pos1.x, pos1.y, pos2.x, pos2.y); }
...
void SWGraphicsInterface::drawRect(int x, int y, int width, int height, Color top, Color right, Color bottom,
                                   Color left) {
    setColor(top);
    drawLine(x, y, x + width, y);
    setColor(left);
...
void SWGraphicsInterface::fillGradient(int x, int y, int width, int height, Color topLeftColor, Color topRightColor,
...
void SWGraphicsInterface::drawQuad(Vector2 topLeft, Vector2 topRight, Vector2 bottomRight, Vector2 bottomLeft,
...
void SWGraphicsInterface::setAntialiasing(bool aa) { m_bAntiAliasing = aa; }
...
UString SWGraphicsInterface::getVendor() { return "McEngine"; }

UString SWGraphicsInterface::getModel() { return "SW Rasterizer"; }

UString SWGraphicsInterface::getVersion() { return "0.0"; }

int SWGraphicsInterface::getVRAMTotal() { return -1; }

int SWGraphicsInterface::getVRAMRemaining() { return -1; }
...
RenderTarget *SWGraphicsInterface::createRenderTarget(int x, int y, int width, int height,
                                                      Graphics::MULTISAMPLE_TYPE multiSampleType) {
    return new SWRenderTarget(x, y, width, height, multiSampleType);
}

Shader *SWGraphicsInterface::createShaderFromFile(std::string vertexShaderFilePath,
...
VertexArrayObject *SWGraphicsInterface::createVertexArrayObject(Graphics::PRIMITIVE primitive,
