...
void OpenGLES2Interface::init() {
    ...
    std::string texturedGenericP =
        ...
        "void main() {\n"
        //"	gl_FragColor = col;\n"
        "	gl_FragColor = mix(col, mix(texture2D(tex, texcoords) * col, texcolor, clamp(type - 1.0, 0.0, 1.0)), "
        "clamp(type, 0.0, 1.0));\n"
        "}\n"
        "\n";
...
void OpenGLES2Interface::clearDepthBuffer() { glClear(GL_DEPTH_BUFFER_BIT); }
...
void OpenGLES2Interface::drawLine(Vector2 pos1, Vector2 pos2) { drawLine(pos1.x, pos1.y, pos2.x, pos2.y); }
...
void OpenGLES2Interface::drawRect(int x, int y, int width, int height, Color top, Color right, Color bottom,
                                  Color left) {
    setColor(top);
    drawLine(x, y, x + width, y);
    setColor(left);
...
void OpenGLES2Interface::fillGradient(int x, int y, int width, int height, Color topLeftColor, Color topRightColor,
...
void OpenGLES2Interface::drawQuad(Vector2 topLeft, Vector2 topRight, Vector2 bottomRight, Vector2 bottomLeft,
...
VertexArrayObject *OpenGLES2Interface::createVertexArrayObject(Graphics::PRIMITIVE primitive,
...
void OpenGLES2Interface::forceUpdateTransform() { updateTransform(); }
