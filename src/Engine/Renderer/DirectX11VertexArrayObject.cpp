...
void DirectX11VertexArrayObject::init() {
            ...
            for(size_t i = 0; i < m_partialUpdateVertexIndices.size(); i++) {
                ...
                while((i + 1) < m_partialUpdateVertexIndices.size()) {
                    if((m_partialUpdateVertexIndices[i + 1] - m_partialUpdateVertexIndices[i]) == 1) {
                        ...
                        m_convertedVertices[m_partialUpdateVertexIndices[i]].pos =
                            m_vertices[m_partialUpdateVertexIndices[i]];
                    } else
                        break;
                }
    ...
    if(m_vertexBuffer != NULL && (!m_bKeepInSystemMemory || m_bReady))
    ...
    m_convertedVertices.clear();
    {
        ...
        for(size_t i = 0; i < m_colors.size(); i++) {
            const Vector4 color = Vector4(COLOR_GET_Rf(m_colors[i]), COLOR_GET_Gf(m_colors[i]),
                                          COLOR_GET_Bf(m_colors[i]), COLOR_GET_Af(m_colors[i]));
...
void DirectX11VertexArrayObject::initAsync() { m_bAsyncReady = true; }
...
int DirectX11VertexArrayObject::usageToDirectX(Graphics::USAGE_TYPE usage) {
    switch(usage) {
        ...
        case Graphics::USAGE_TYPE::USAGE_DYNAMIC:
            return D3D11_USAGE::D3D11_USAGE_DEFAULT;  // NOTE: this is intentional. no performance benefits found so far
                                                      // with DYNAMIC, since D3D11_MAP_WRITE_NO_OVERWRITE has very
                                                      // limited use cases
