...
OpenCLInterface::OpenCLInterface() {
    ...
    const cl_platform_info attributeTypes[numAttributes] = {CL_PLATFORM_NAME, CL_PLATFORM_VENDOR, CL_PLATFORM_VERSION,
...
OpenCLInterface::~OpenCLInterface() {
    ...
    if(m_commandQueue != NULL) {
        ...
        if(ret != CL_SUCCESS)
            engine->showMessageError("OpenCL Error",
                                     UString::format("clReleaseCommandQueue(m_commandQueue) returned %i", ret));
    }
...
void OpenCLInterface::cleanup() {
    ...
    for(size_t i = 0; i < m_vBuffers.size(); i++) {
        if(m_vBuffers[i] != NULL) {
            ...
            if(ret != CL_SUCCESS)
                engine->showMessageError("OpenCL Error",
                                         UString::format("clReleaseMemObject(m_vBuffers[%i]) returned %i", i, ret));
        }
    }
    m_vBuffers.clear();
    ...
    for(size_t i = 0; i < m_vKernels.size(); i++) {
        ...
        if(ret != CL_SUCCESS)
            engine->showMessageError("OpenCL Error",
                                     UString::format("clReleaseKernel(m_vKernels[%i]) returned %i", i, ret));
    }
    m_vKernels.clear();
    ...
    for(size_t i = 0; i < m_vPrograms.size(); i++) {
        ...
        if(ret != CL_SUCCESS)
            engine->showMessageError("OpenCL Error",
                                     UString::format("clReleaseProgram(m_vPrograms[%i]) returned %i", i, ret));
    }
    m_vPrograms.clear();
...
void OpenCLInterface::releaseKernel(int kernel) {
    ...
    if(ret != CL_SUCCESS)
        engine->showMessageError("OpenCL Error",
                                 UString::format("clReleaseKernel(m_vKernels[%i]) returned %i", kernel, ret));
    ...
    if(ret != CL_SUCCESS)
        engine->showMessageError("OpenCL Error",
                                 UString::format("clReleaseProgram(m_vKernels[%i]) returned %i", kernel, ret));
...
int OpenCLInterface::createKernel(UString kernelSourceCode, UString functionName) {
    ...
    cl_program program =
        clCreateProgramWithSource(m_context, 1, (const char **)&kernelSource, (const size_t *)&kernelLength, &ret);
    ...
    if(ret == CL_BUILD_PROGRAM_FAILURE) {
        ...
        log[logSize] = '\0';
        debugLog("%s\n", log);
        env->showMessageWarning(
            "OpenCL Compiler Error",
            "Either a syntax error or an OpenCL compiler compatibility error, take a look at the console for details.");
        delete[] log;
    ...
    if(ret != CL_SUCCESS) {
        engine->showMessageError("OpenCL Error",
                                 UString::format("clBuildProgram() returned %i! (possible syntax error?)", ret));
        return -1;
    }
...
int OpenCLInterface::createKernel(const char *kernelSourceCode, UString functionName) {
    ...
    if(ret == CL_BUILD_PROGRAM_FAILURE) {
        ...
        log[logSize] = '\0';
        debugLog("%s\n", log);
        env->showMessageWarning("OpenCL Compiler Error",
                                "Probably a syntax error, take a look at the console for details");
        delete[] log;
...
void OpenCLInterface::setKernelArg(int kernel, unsigned int argumentNumber, int buffer) {
    ...
    if(kernel < 0 || kernel > (int)(m_vKernels.size() - 1) || argumentNumber < 0 || buffer < 0 ||
       buffer > (int)(m_vBuffers.size() - 1)) {
        engine->showMessageError("OpenCL Error", UString::format("Illegal arguments to setKernelArg(%i, %i, %i)!",
                                                                 kernel, argumentNumber, buffer));
        return;
    }
    ...
    if(ret != CL_SUCCESS)
        engine->showMessageError("OpenCL Error", UString::format("clSetKernelArg(%i, %i, %i) returned %i!", kernel,
                                                                 argumentNumber, buffer, ret));
#endif
}

void OpenCLInterface::executeKernel(int kernel, unsigned int numLoops, const size_t *globalItemSize,
                                    const size_t *localItemSize) {
#ifdef MCENGINE_FEATURE_OPENCL

    if(kernel < 0 || kernel > (int)(m_vKernels.size() - 1) || globalItemSize == NULL || localItemSize == NULL) {
        engine->showMessageError("OpenCL Error", UString::format("Illegal arguments to executeKernel(%i, %i, %i)!",
                                                                 kernel, globalItemSize, localItemSize));
        return;
    }

    cl_int ret = CL_INVALID_VALUE;
    ret = clEnqueueNDRangeKernel(m_commandQueue, m_vKernels[kernel], numLoops, NULL, globalItemSize, localItemSize, 0,
                                 NULL, NULL);
    ...
    if(ret != CL_SUCCESS) {
        if(ret == -1000)
            engine->showMessageError(
                "OpenCL Error",
                UString::format(
                    "clEnqueueNDRangeKernel(%i, %i, %i) returned %i!\nYou probably forgot to opencl->aquireGLObject()!",
                    kernel, globalItemSize, localItemSize, ret));
        else
            engine->showMessageError("OpenCL Error",
                                     UString::format("clEnqueueNDRangeKernel(%i, %i, %i) returned %i!\n\n(You can quit "
                                                     "the engine by closing the console!)",
                                                     kernel, globalItemSize, localItemSize, ret));
    }
...
void OpenCLInterface::aquireGLObject(int object) {
    ...
    if(m_vBufferIndex[object] > (int)(m_vBuffers.size() - 1) || m_vBufferIndex[object] < 0) {
        engine->showMessageError("OpenCL Error",
                                 UString::format("Illegal arguments to aquireGLObject(%i)!", m_vBufferIndex[object]));
        return;
    }
    ...
    if(ret != CL_SUCCESS)
        engine->showMessageError("OpenCL Error",
                                 UString::format("clEnqueueAcquireGLObjects(%i) returned %i!\n", object));
...
void OpenCLInterface::releaseGLObject(int object) {
    ...
    if(m_vBufferIndex[object] > (int)(m_vBuffers.size() - 1) || m_vBufferIndex[object] < 0) {
        engine->showMessageError("OpenCL Error",
                                 UString::format("Illegal arguments to releaseGLObject(%i)!", m_vBufferIndex[object]));
        return;
    }
    ...
    if(ret != CL_SUCCESS)
        engine->showMessageError("OpenCL Error",
                                 UString::format("clEnqueueReleaseGLObjects(%i) returned %i!\n", object));
...
int OpenCLInterface::createTexture(unsigned int sourceImage, bool readable, bool writeable) {
    ...
    cl_mem img = clCreateFromGLTexture2D(
        m_context,
        (readable && writeable) ? CL_MEM_READ_WRITE : ((readable && !writeable) ? CL_MEM_READ_ONLY : CL_MEM_WRITE_ONLY),
        GL_TEXTURE_2D, 0, sourceImage, &error);

    if(!img || error) {
        engine->showMessageError("OpenCL Error",
                                 UString::format("clCreateFromGLTexture2D() returned %i, error = %i\n", img, error));
        return -1;
    }
...
int OpenCLInterface::createTexture3D(unsigned int sourceImage) {
    ...
    if(!img || error) {
        engine->showMessageError(
            "OpenCL Error",
            UString::format("clCreateFromGLTexture3D(%i) returned %i, error = %i\n", sourceImage, img, error));
        return -1;
    }
...
int OpenCLInterface::getWorkGroupSize(int kernel) {
    ...
    if(kernel < 0 || kernel > (int)(m_vKernels.size() - 1)) return -1;

    size_t size;
    int ret = clGetKernelWorkGroupInfo(m_vKernels[kernel], m_deviceID, CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t),
                                       (void *)&size, NULL);
...
void OpenCLInterface::releaseBuffer(int buffer) {
    ...
    if(buffer > -1 || buffer < m_vBufferIndex.size()) {
        if(m_vBufferIndex[buffer] > m_vBuffers.size() || m_vBufferIndex[buffer] < 0) {
            engine->showMessageError(
                "OpenCL Error", UString::format("Invalid Buffer for releaseBuffer(%i)!\n", m_vBufferIndex[buffer]));
            return;
        }

        cl_int ret = clReleaseMemObject(m_vBuffers[m_vBufferIndex[buffer]]);
        m_vBuffers[m_vBufferIndex[buffer]] =
            NULL;  // HACKHACK: can't erase() because then the indices would change, breaking all existing buffers!
        debugLog("OpenCL: clReleaseMemObject(m_vBuffers[%i]) returned %i!\n", buffer, ret);
        if(ret != CL_SUCCESS)
            engine->showMessageError("OpenCL Error",
                                     UString::format("clReleaseMemObject(m_vBuffers[%i]) returned %i", buffer, ret));
    }
