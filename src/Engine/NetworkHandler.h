...
class NetworkHandler {
   ...
   public:
    ...
    typedef fastdelegate::FastDelegate0<>
        NetworkLocalServerStartedListener;  // called when we successfully start the server
    typedef fastdelegate::FastDelegate0<> NetworkLocalServerStoppedListener;  // called when the host stops

    typedef fastdelegate::FastDelegate0<>
        NetworkClientDisconnectedFromServerListener;  // called when the connection is stopped, (e.g. if the host is
                                                      // stopped, or you disconnect)
    typedef fastdelegate::FastDelegate0<>
        NetworkClientConnectedToServerListener;  // called when a connection has been established (if the client has
                                                 // successfully connected)

    typedef fastdelegate::FastDelegate0<EXTENSION_PACKET>
        NetworkClientSendInfoListener;  // allows clients to send custom information to the server, independent of the
                                        // NetworkHandler
    typedef fastdelegate::FastDelegate0<EXTENSION_PACKET>
        NetworkServerSendInfoListener;  // allows the server to send custom information to the client, independent of
                                        // the NetworkHandler

    // returning false on either one of these will terminate the connection:
    typedef fastdelegate::FastDelegate1<void *, bool>
        NetworkClientReceiveServerInfoListener;  // called with the data of the EXTENSION_PACKET (if used) upon
                                                 // receiving the server info packet
    typedef fastdelegate::FastDelegate1<void *, bool>
        NetworkServerReceiveClientInfoListener;  // called with the data of the EXTENSION_PACKET (if used) upon
                                                 // receiving a client info packet

    typedef fastdelegate::FastDelegate3<uint32_t, void *, uint32_t, bool>
        NetworkServerReceiveClientPacketListener;  // packets sent through this class' functions will be received here
    typedef fastdelegate::FastDelegate3<uint32_t, void *, uint32_t, bool>
        NetworkClientReceiveServerPacketListener;  // packets sent through this class' functions will be received here
    typedef fastdelegate::FastDelegate3<uint32_t, UString, bool>
        NetworkServerClientChangeListener;  // called when a client's connection state (or another state of it, like its
                                            // name or something) changes
    ...
    void broadcast(
        ...
        bool reliable = false);  // two functions depending on the callee: [server -> all clients] or [client -> server
                                 // -> all clients]; if both server and client, server wins (first case)
    void servercast(void *data, uint32_t size, bool reliable = false);  // [client -> server]; only valid as client
    void clientcast(void *data, uint32_t size, uint32_t id,
                    bool reliable = false);  // [server -> client]; only valid as server

    // callbacks
    void setOnClientReceiveServerPacketListener(NetworkClientReceiveServerPacketListener listener) {
        m_clientReceiveServerPacketListener = listener;
    }
    void setOnServerReceiveClientPacketListener(NetworkServerReceiveClientPacketListener listener) {
        m_serverReceiveClientPacketListener = listener;
    }
    void setOnServerClientChangeListener(NetworkServerClientChangeListener listener) {
        m_serverClientChangeListener = listener;
    }
    ...
    void setOnClientReceiveServerInfoListener(NetworkClientReceiveServerInfoListener listener) {
        m_clientReceiveServerInfoListener = listener;
    }
    void setOnServerReceiveClientInfoListener(NetworkServerReceiveClientInfoListener listener) {
        m_serverReceiveClientInfoListener = listener;
    }

    void setOnClientConnectedToServerListener(NetworkClientConnectedToServerListener listener) {
        m_clientConnectedToServerListener = listener;
    }
    void setOnClientDisconnectedFromServerListener(NetworkClientDisconnectedFromServerListener listener) {
        m_clientDisconnectedFromServerListener = listener;
    }

    void setOnLocalServerStartedListener(NetworkLocalServerStartedListener listener) {
        m_localServerStartedListener = listener;
    }
    void setOnLocalServerStoppedListener(NetworkLocalServerStoppedListener listener) {
        m_localServerStoppedListener = listener;
    }
   ...
   private:
    ...
    enum PACKET_TYPE {
        ...
        CLIENT_BROADCAST_PACKET_TYPE,  // client -> server -> all other clients (or, if we are the server: server -> all
                                       // other clients)
    ...
    void sendServerInfo(uint32_t assignedID, ENetHost *host,
                        ENetPeer *destination);  // server -> client (on a successful connection)
    ...
    struct CLIENT_PEER {
        ENetPeer *peer;
        UString name;
        float kickTime;      // if the client has been kicked, this serves as a delay to allow the kick chat message to
                             // arrive properly
...
#pragma pack(1)
    ...
    struct SERVER_INFO_PACKET {
        ...
        uint32_t id;
        bool extension;  // if there is a custom packet of the current app, which may be appended to this one by the
                         // onServerSendInfo() API
    };
