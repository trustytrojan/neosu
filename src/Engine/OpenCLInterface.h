...
int OpenCLInterface::createBuffer(unsigned int numberOfObjects, bool readable, bool writeable) {
    ...
    if(numberOfObjects < 1) {
        engine->showMessageError(
            "OpenCL Error",
            UString::format("Can't createBuffer() with %i numberOfObjects you idiot!\n", numberOfObjects));
        return -1;
    }
    ...
    if(requestedMemSizeInMB > m_iMaxMemAllocSizeInMB) {
        debugLog(
            "OpenCL Warning: Trying to allocate %i MB for a single buffer, but the maximum supported size is %i MB!\n",
            requestedMemSizeInMB, m_iMaxMemAllocSizeInMB);
        // engine->showInfo("Memory Warning", UString::format("Trying to allocate %i MB for a single buffer, but the
        // maximum supported size is %i MB.\n", requestedMemSizeInMB, m_iMaxMemAllocSizeInMB));
    }

    cl_int ret = CL_INVALID_VALUE;
    cl_mem bufferObject = NULL;
    bufferObject = clCreateBuffer(
        m_context,
        (readable && writeable) ? CL_MEM_READ_WRITE : ((readable && !writeable) ? CL_MEM_READ_ONLY : CL_MEM_WRITE_ONLY),
        numberOfObjects * sizeof(T), NULL, &ret);
...
void OpenCLInterface::writeBuffer(int buffer, unsigned int numberOfObjects, const void *ptr) {
    ...
    if(buffer < 0 || buffer > (int)(m_vBufferIndex.size() - 1) || numberOfObjects < 1 || ptr == NULL) {
        engine->showMessageError("OpenCL Error", UString::format("Illegal arguments to writeBuffer(%i, %i, [ptr])!",
                                                                 buffer, numberOfObjects));
        return;
    }

    if(m_vBufferIndex[buffer] > (int)(m_vBuffers.size() - 1) || m_vBufferIndex[buffer] < 0) {
        engine->showMessageError("OpenCL Error", UString::format("Illegal arguments to writeBuffer(%i, %i,[ptr])!",
                                                                 m_vBufferIndex[buffer], numberOfObjects));
        return;
    }

    cl_int ret = CL_INVALID_VALUE;
    ret = clEnqueueWriteBuffer(m_commandQueue, m_vBuffers[m_vBufferIndex[buffer]], CL_TRUE, 0,
                               numberOfObjects * sizeof(T), ptr, 0, NULL, NULL);

    if(ret != CL_SUCCESS)
        engine->showMessageError(
            "OpenCL Error",
            UString::format("clEnqueueWriteBuffer() returned %i!\n\n(You can quit the engine by closing the console!)",
                            ret));
...
template <typename T>
void OpenCLInterface::writeBuffer(int buffer, unsigned int numberOfObjects, unsigned int startObjectIndex,
                                  const void *ptr) {
#ifdef MCENGINE_FEATURE_OPENCL

    if(buffer < 0 || buffer > (int)(m_vBufferIndex.size() - 1) || numberOfObjects < 1 || ptr == NULL ||
       startObjectIndex < 0) {
        engine->showMessageError("OpenCL Error", UString::format("Illegal arguments to writeBuffer(%i, %i, %i, [ptr])!",
                                                                 buffer, numberOfObjects, startObjectIndex));
        return;
    }

    if(m_vBufferIndex[buffer] > (int)(m_vBuffers.size() - 1) || m_vBufferIndex[buffer] < 0) {
        engine->showMessageError("OpenCL Error",
                                 UString::format("Illegal arguments to writeBuffer(%i, %i, %i, [ptr])!",
                                                 m_vBufferIndex[buffer], numberOfObjects, startObjectIndex));
        return;
    }

    cl_int ret = CL_INVALID_VALUE;
    ret = clEnqueueWriteBuffer(m_commandQueue, m_vBuffers[m_vBufferIndex[buffer]], CL_TRUE,
                               startObjectIndex * sizeof(T), numberOfObjects * sizeof(T), ptr, 0, NULL, NULL);
...
void OpenCLInterface::readBuffer(int buffer, unsigned int numberOfObjects, void *ptr) {
    ...
    if(buffer < 0 || buffer > (int)(m_vBufferIndex.size() - 1) || numberOfObjects < 1 || ptr == NULL) {
        engine->showMessageError(
            "OpenCL Error", UString::format("Illegal arguments to readBuffer(%i, [ptr])!", buffer, numberOfObjects));
        return;
    }

    if(m_vBufferIndex[buffer] > (int)(m_vBuffers.size() - 1) || m_vBufferIndex[buffer] < 0) {
        engine->showMessageError("OpenCL Error", UString::format("Illegal arguments to readBuffer(%i, [ptr])!",
                                                                 m_vBufferIndex[buffer], numberOfObjects));
        return;
    }

    cl_int ret = CL_INVALID_VALUE;
    ret = clEnqueueReadBuffer(m_commandQueue, m_vBuffers[m_vBufferIndex[buffer]], CL_TRUE, 0,
                              numberOfObjects * sizeof(T), ptr, 0, NULL, NULL);
    ...
    if(ret != CL_SUCCESS)
        engine->showMessageError(
            "OpenCL Error",
            UString::format(
                "clEnqueueReadBuffer() returned %i!\nProbably an out-of-bounds memory read/write in the kernel!!!",
                ret));
...
void OpenCLInterface::updateBuffer(int buffer, unsigned int numberOfObjects, bool readable, bool writeable) {
    ...
    if(buffer < 0 || buffer > (int)(m_vBufferIndex.size() - 1) || numberOfObjects < 1) {
        engine->showMessageError(
            "OpenCL Error", UString::format("Illegal arguments to updateBuffer(%i, %i)!", buffer, numberOfObjects));
        return;
    }
...
void OpenCLInterface::setKernelArg(int kernel, unsigned int argumentNumber, T argument) {
    ...
    if(kernel < 0 || kernel > (int)(m_vKernels.size() - 1) || argumentNumber < 0) {
        engine->showMessageError(
            "OpenCL Error",
            UString::format("Illegal arguments to setKernelArg(%i, %i, T [template])!", kernel, argumentNumber));
        return;
    }
    ...
    if(ret != CL_SUCCESS)
        engine->showMessageError("OpenCL Error",
                                 UString::format("clSetKernelArg(%i, %i, T [template]) returned %i!\n\n(You can quit "
                                                 "the engine by closing the console!)",
                                                 kernel, argumentNumber, ret));
