...
Graphics *MacOSEnvironment::createRenderer() { return new MacOSGLLegacyInterface(); }
...
void MacOSEnvironment::sleep(unsigned int us) { MacOSWrapper::microSleep(us); }
...
void MacOSEnvironment::openURLInDefaultBrowser(UString url) { MacOSWrapper::openURLInDefaultBrowser(url.toUtf8()); }

UString MacOSEnvironment::getUsername() { return UString(MacOSWrapper::getUsername()); }
...
bool MacOSEnvironment::deleteFile(std::string filePath) { return remove(filePath.c_str()) == 0; }
...
void MacOSEnvironment::setClipBoardText(UString text) { MacOSWrapper::setClipboardText(text.toUtf8()); }
...
void MacOSEnvironment::focus() { MacOSWrapper::focus(); }

void MacOSEnvironment::center() { MacOSWrapper::center(); }

void MacOSEnvironment::minimize() { MacOSWrapper::minimize(); }

void MacOSEnvironment::maximize() { MacOSWrapper::maximize(); }
...
void MacOSEnvironment::setWindowTitle(UString title) { MacOSWrapper::setWindowTitle(title.toUtf8()); }

void MacOSEnvironment::setWindowPos(int x, int y) { MacOSWrapper::setWindowPos(x, y); }

void MacOSEnvironment::setWindowSize(int width, int height) { MacOSWrapper::setWindowSize(width, height); }

void MacOSEnvironment::setWindowResizable(bool resizable) { MacOSWrapper::setWindowResizable(resizable); }
...
int MacOSEnvironment::getMonitor() { return MacOSWrapper::getMonitor(); }

std::vector<McRect> MacOSEnvironment::getMonitors() { return m_vMonitors; }
...
bool MacOSEnvironment::isCursorInWindow() { return m_bIsCursorInsideWindow; }

bool MacOSEnvironment::isCursorVisible() { return m_bCursorVisible; }

Vector2 MacOSEnvironment::getMousePos() {
    MacOSWrapper::VECTOR2 mousePos = MacOSWrapper::getMousePos();
    return Vector2(mousePos.x,
                   -mousePos.y + engine->getScreenHeight() -
                       1);  // wtf apple: "The y coordinate in the returned point starts from a base of 1, not 0."
}

McRect MacOSEnvironment::getCursorClip() { return m_cursorClip; }

CURSORTYPE MacOSEnvironment::getCursor() { return m_cursorType; }
...
void MacOSEnvironment::setCursorVisible(bool visible) { setCursorVisible(visible, false); }
...
void MacOSEnvironment::setCursorVisible(bool visible, bool internalOverride) {
    ...
    if(!internalOverride) m_bCursorVisible = visible;

    m_bCursorVisibleInternalOverride = visible;
}

void MacOSEnvironment::setMousePos(int x, int y) { MacOSWrapper::setMousePos(x, y); }
...
int MacOSEnvironment::getFilesInFolderFilter(const struct dirent *entry) { return 1; }

int MacOSEnvironment::getFoldersInFolderFilter(const struct dirent *entry) { return 1; }
