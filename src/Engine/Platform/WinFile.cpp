...
WinFile::WinFile(std::string filePath, File::TYPE type) {
    ...
    if(File::debug->getBool()) debugLog("WinFile: Opening %s, m_iFileSize = %i\n", filePath.c_str(), m_iFileSize);
...
bool WinFile::canRead() const { return m_bReady && m_bCanRead; }

bool WinFile::canWrite() const { return m_bReady && m_bCanWrite; }
...
bool WinFile::checkReadForLineBuffer() {
    ...
    if(m_buffer == NULL) m_buffer = new char[bufferSize];
...
UString WinFile::readLine() {
    ...
    while(!foundLine && m_bCanRead) {
        ...
        for(size_t i = m_iLineIndex;
            (i < bufferSize) && ((i + m_iLineBufferReadIndexOffset) < m_iRead) && (lineBufferIndex < bufferSize); i++) {
            ...
            if(c == '\r') {
                ...
                if(i + m_iLineBufferReadIndexOffset + 1 < m_iRead &&
                   ((unsigned char *)m_buffer)[i + m_iLineBufferReadIndexOffset + 1] == '\n')
                    i++;
        ...
        if(lineBufferIndex > 0 || foundLine) {
            ...
            if(!foundLine) m_iLineIndex += lineBufferIndex;  // increase read position for possible next line

            // only append if we have anything at all
            if(lineBufferIndex > 0) result.append(lineBuffer);
...
const char *WinFile::readFile() {
    if(File::debug->getBool()) debugLog("WinFile::readFile() on %s\n", m_sFilePath.c_str());
...
size_t WinFile::getFileSize() const { return m_iFileSize; }
