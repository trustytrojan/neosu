...
#error "TODO: add correct extension here"
    ...
    */
    ...
    for(size_t i = 0; i < availableLayers.size(); i++) {
        debugLog("Vulkan: Layer %i = %s v%i (%s)\n", i, availableLayers[i].description,
                 availableLayers[i].implementationVersion, availableLayers[i].layerName);
    }
    ...
    if(res != VK_SUCCESS) {
        if(res == VK_ERROR_EXTENSION_NOT_PRESENT)
            engine->showMessageError(
                "Vulkan Error",
                UString::format("Couldn't vkCreateInstance(), returned %d. Required Vulkan extensions are not present!",
                                res));
        else if(res == VK_ERROR_LAYER_NOT_PRESENT)
            engine->showMessageError(
                "Vulkan Error",
                UString::format("Couldn't vkCreateInstance(), returned %d. Required Vulkan layers are not present!",
                                res));
        else
    ...
    if(res != VK_SUCCESS) {
        engine->showMessageError("Vulkan Error",
                                 UString::format("Couldn't vkEnumeratePhysicalDevices(), returned %d", res));
        return;
    }
    if(numDevices < 1) {
    ...
    if(queueFamilyPropertyCount < 1) {
        engine->showMessageError(
            "Vulkan Error",
            UString::format("vkGetPhysicalDeviceQueueFamilyProperties() returned %i queueFamilyProperties",
