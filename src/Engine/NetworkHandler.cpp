...
ConVar connect_duration("connect_duration", 5.0f, FCVAR_NONE,
                        "Time in seconds to wait for a response from the server when trying to connect");
ConVar _disconnect_("disconnect");
ConVar disconnect_duration("disconnect_duration", 3.0f, FCVAR_NONE,
                           "Time in seconds to wait for a gentle disconnect before dropping the connection");
...
ConVar cl_updaterate("cl_updaterate", 66.0f, FCVAR_NONE,
                     "How many snapshots/updates/deltas are requested from the server per second");
...
NetworkHandler::~NetworkHandler() {
    ...
    if(m_clientPeer != NULL) enet_peer_reset(m_clientPeer);
    if(m_client != NULL) enet_host_destroy(m_client);
    if(isServer()) enet_host_destroy(m_server);
...
void NetworkHandler::connect(UString address) {
    ...
    if(address.length() < 1) address = "localhost";
    ...
    // create the client
    m_client = enet_host_create(NULL /* create a client host */, 1 /* only allow 1 outgoing connection */,
                                2 /* allow up 2 channels to be used, 0 and 1 */, 0 /* unlimited downstream bandwidth */,
...
void NetworkHandler::host() {
    ...
    m_server = enet_host_create(&address, host_max_clients.getInt(), 2 /* allow up to 2 channels to be used, 0 and 1 */,
                                0 /* unlimited downstream bandwidth */, 0 /* unlimited upstream bandwidth */);
    ...
    if(m_localServerStartedListener != NULL) m_localServerStartedListener();
...
void NetworkHandler::hostStop() {
    ...
    // notify listener
    if(m_localServerStoppedListener != NULL) m_localServerStoppedListener();
    ...
    if(isServer()) {
        ...
        if(m_vConnectedClients.size() > 0) {
            for(size_t c = 0; c < m_vConnectedClients.size(); c++) {
                ...
                if(m_vConnectedClients[c].kickTime != 0.0f &&
                   engine->getTime() > m_vConnectedClients[c].kickTime + 1.0f) {
                ...
                if(m_vConnectedClients[c].kickKillTime != 0.0f &&
                   engine->getTime() > m_vConnectedClients[c].kickKillTime + 1.0f) {
                ...
                switch(*((PACKET_TYPE *)e.packet->data)) {
                    case SERVER_INFO_PACKET_TYPE:
                        if(e.packet->dataLength >= sizeof(SERVER_INFO_PACKET)) {
                            ...
                            if(m_clientReceiveServerInfoListener != NULL && sp->extension)
                                valid = m_clientReceiveServerInfoListener(
                                    (void *)(e.packet->data + sizeof(SERVER_INFO_PACKET)));

                            // this is the last packet we needed before starting the game
                            if(valid) {
                                if(m_clientConnectedToServerListener != NULL) m_clientConnectedToServerListener();
                            } else
                    ...
                    case CHAT_PACKET_TYPE:
                        if(e.packet->dataLength >= sizeof(CHAT_PACKET)) {
                            if(!isServer())  // to avoid double logs on local hosts, since the client has already
                                             // received the message through onServerEvent (and can't send it to itself
                                             // in the broadcast)
                            {
                                chatLog(UString(((struct CHAT_PACKET *)e.packet->data)->username)
                                            .substr(0, ((struct CHAT_PACKET *)e.packet->data)->usize),
                                        UString(((struct CHAT_PACKET *)e.packet->data)->message)
                                            .substr(0, ((struct CHAT_PACKET *)e.packet->data)->msize));
                            }
                        }
                        break;
                    ...
                    case CLIENT_BROADCAST_PACKET_TYPE:
                        if(e.packet->dataLength >= sizeof(CLIENT_BROADCAST_WRAPPER)) {
                            if(m_clientReceiveServerPacketListener != NULL) {
                                ...
                                // process
                                if(!m_clientReceiveServerPacketListener(wrapper->id, unwrappedPacket,
                                                                        e.packet->dataLength - wrapperSize))
    ...
    switch(e.type) {
                ...
                switch(*((PACKET_TYPE *)e.packet->data)) {
                    case CLIENT_INFO_PACKET_TYPE:
                        if(e.packet->dataLength >= sizeof(CLIENT_INFO_PACKET)) {
                            CLIENT_INFO_PACKET *cp =
                                new CLIENT_INFO_PACKET();  // 'new' here because this is stored in e.peer->data
                            *cp = *(struct CLIENT_INFO_PACKET *)e.packet->data;  // TODO: wtf is even happening here
                            ...
                            if(pp != NULL) {
                                pp->name = UString(((struct CLIENT_INFO_PACKET *)e.peer->data)->username)
                                               .substr(0, ((struct CLIENT_INFO_PACKET *)e.peer->data)->size);

                                // if the client is not running the same version
                                if(cp->version != MC_PROTOCOL_VERSION) {
                                    debugLog(
                                        "SERVER: User is trying to connect using version %i, but the server is running "
                                        "version %i.\n",
                                        cp->version, MC_PROTOCOL_VERSION);
                                    singlecastChatMessage("CONSOLE",
                                                          UString::format("Version mismatch: Server is running version "
                                                                          "%i, but you are running version %i!",
                                                                          MC_PROTOCOL_VERSION, cp->version),
                                                          m_server, e.peer);
                                    ...
                                    if(m_serverReceiveClientInfoListener != NULL && cp->extension)
                                        valid = m_serverReceiveClientInfoListener(
                                            (void *)(e.packet->data + sizeof(CLIENT_INFO_PACKET)));
                                    ...
                                    if(valid) {
                                        ...
                                        pp->kickTime = engine->getTime();  // if the extension packet listener returned
                                                                           // false, kick this client
                                }
                            } else
                    ...
                    case CHAT_PACKET_TYPE:
                        if(e.packet->dataLength >= sizeof(CHAT_PACKET)) {
                            ...
                            if(pp != NULL) {
                                ...
                                UString message = UString(((struct CHAT_PACKET *)e.packet->data)->message)
                                                      .substr(0, ((struct CHAT_PACKET *)e.packet->data)->msize);
                    ...
                    case CLIENT_BROADCAST_PACKET_TYPE:
                        if(e.packet->dataLength >= sizeof(PACKET_TYPE)) {
                            // HACKHACK: TODO: this is dangerous, broadcasting all client broadcast requests by default
                            // if no server listener is set
                            bool accepted = true;
                            ...
                            if(m_serverReceiveClientPacketListener != NULL) {
                                ...
                                if(pp != NULL) {
                                    ...
                                    // process
                                    accepted = m_serverReceiveClientPacketListener(pp->id, unwrappedPacket,
                                                                                   e.packet->dataLength - wrapperSize);
                                } else
                                    debugLog("SERVER: NULL CLIENT_PEER!\n");
                            }
                            ...
                            if(accepted && m_vConnectedClients.size() > 0) {
                                ...
                                const int wrapperSize = sizeof(CLIENT_BROADCAST_WRAPPER);
                                char wrappedPacket[(sizeof(CLIENT_BROADCAST_WRAPPER) + e.packet->dataLength -
                                                    unWrapperSize)];
                                // the id is not set here, but for every client in the loop below
                                memcpy(((char *)&wrappedPacket) + wrapperSize, unwrappedPacket,
                                       (e.packet->dataLength - unWrapperSize));
                        ...
                        if(e.packet->dataLength >= sizeof(PACKET_TYPE)) {
                            if(m_serverReceiveClientPacketListener != NULL) {
                                ...
                                if(pp != NULL) {
                                    ...
                                    // process
                                    if(!m_serverReceiveClientPacketListener(pp->id, unwrappedPacket,
                                                                            e.packet->dataLength - wrapperSize))
        ...
        case ENET_EVENT_TYPE_DISCONNECT:
            ...
            if(pp != NULL) {
                ...
                // notify local connection listener
                if(m_serverClientChangeListener != NULL) m_serverClientChangeListener(pp->id, UString(""), false);
            } else
                debugLog("SERVER: NULL CLIENT_PEER!\n");
...
void NetworkHandler::clientDisconnect() {
    ...
    // notify listener
    if(isClient() && m_clientDisconnectedFromServerListener != NULL) m_clientDisconnectedFromServerListener();
...
void NetworkHandler::broadcastChatMessage(CHAT_PACKET *cp, ENetHost *host, ENetPeer *origin) {
    for(size_t c = 0; c < m_vConnectedClients.size(); c++) {
        if(m_vConnectedClients[c].peer != origin) singlecastChatMessage(cp, host, m_vConnectedClients[c].peer);
    }
}
...
void NetworkHandler::broadcast(void *data, uint32_t size, bool reliable) {
    ...
    if(isServer() && m_vConnectedClients.size() > 0) {
        ...
        for(size_t c = 0; c < m_vConnectedClients.size(); c++) {
            ...
            // create it
            ENetPacket *packet =
                enet_packet_create((const void *)wrappedPacket, size, reliable ? ENET_PACKET_FLAG_RELIABLE : 0);

            // send it
            if(packet != NULL) enet_peer_send(m_vConnectedClients[c].peer, 0, packet);
        }
...
void NetworkHandler::kick(UString username) {
    ...
    for(size_t c = 0; c < m_vConnectedClients.size(); c++) {
        if(m_vConnectedClients[c].name == username) {
            // send kick message
            singlecastChatMessage("CONSOLE", "You have been KICKED from the server!", m_server,
                                  m_vConnectedClients[c].peer);

            // initiate graceful disconnect
            m_vConnectedClients[c].kickTime = engine->getTime();

            // notify local connection listener
            // TODO: this gets called in the disconnect event, or in the timeout (is it really necessary to also
            // immediately call it here? results in a redundant second message to every client)
            /*
            if (m_serverClientChangeListener != NULL)
...
NetworkHandler::CLIENT_PEER *NetworkHandler::getClientPeerByPeer(ENetPeer *peer) {
    for(size_t c = 0; c < m_vConnectedClients.size(); c++) {
        if(m_vConnectedClients[c].peer == peer) return (&m_vConnectedClients[c]);
    }
    return NULL;
}
...
NetworkHandler::CLIENT_PEER *NetworkHandler::getClientPeerById(uint32_t id) {
    for(size_t c = 0; c < m_vConnectedClients.size(); c++) {
        if(m_vConnectedClients[c].id == id) return (&m_vConnectedClients[c]);
    }
    return NULL;
}
