...
OsuUISearchOverlay::OsuUISearchOverlay(Osu *osu, float xPos, float yPos, float xSize, float ySize, UString name)
...
void OsuUISearchOverlay::draw(Graphics *g) {
    ...
    */
    ...
    const int offsetTextWidthWithoutOverflow =
        offsetTextStringWidth * searchTextScale + (searchTextFont->getHeight() * searchTextScale) + m_iOffsetRight;

    // calc global x offset for overflowing line (don't wrap, just move everything to the left)
    int textOverflowXOffset = 0;
    {
        const int actualXEnd = (int)(m_vPos.x + m_vSize.x - offsetTextStringWidth * searchTextScale -
    ...
    // draw background
    {
        ...
        float numLines = 1.0f;
        {
            ...
            if(m_sHardcodedSearchString.length() > 0) numLines += 1.5f;
        }
    ...
    g->pushTransform();
    {
        ...
        g->translate(
            (int)(m_vPos.x + m_vSize.x - offsetTextStringWidth * searchTextScale -
                  (searchTextFont->getHeight() * searchTextScale) * 0.5f - m_iOffsetRight - textOverflowXOffset),
            (int)(m_vPos.y + (searchTextFont->getHeight() * searchTextScale) * 1.5f));
        ...
        if(hasSearchSubTextVisible) {
            g->translate(0, (int)((searchTextFont->getHeight() * searchTextScale) * 1.5f *
                                  (m_sHardcodedSearchString.length() > 0 ? 2.0f : 1.0f)));
            g->translate(1, 1);
            ...
            if(m_bSearching) {
                ...
                g->drawString(
                    searchTextFont,
                    m_iNumFoundResults > -1
                        ? (m_iNumFoundResults > 0
                               ? UString::format(m_iNumFoundResults == 1 ? "%i match found!" : "%i matches found!",
                                                 m_iNumFoundResults)
                               : noMatchesFoundText1)
                        : noMatchesFoundText2);
                g->translate(-1, -1);
