...
Vector3 OsuBeatmapStandard::osuCoordsTo3D(Vector2 coords, const OsuHitObject *hitObject) const {
    ...
    const float xCurvePercent = (1.0f + ((coords.x / ((float)OsuGameRules::OSU_COORD_WIDTH / 2.0f)) *
                                         m_fposu_3d_curve_multiplier_ref->getFloat())) /
                                2.0f;

    Vector3 coords3d =
        Vector3(coords.x, -coords.y, 0) * OsuModFPoSu::SIZEDIV3D * m_osu->getFPoSu()->get3DPlayfieldScale();

    if(m_fposu_mod_strafing_ref->getBool()) {
        const float speedMultiplierCompensation = 1.0f / getSpeedMultiplier();

        const float x = std::sin((m_iCurMusicPos / 1000.0f) * 5 * speedMultiplierCompensation *
                                 m_fposu_mod_strafing_frequency_x_ref->getFloat()) *
                        m_fposu_mod_strafing_strength_x_ref->getFloat();
        const float y = std::sin((m_iCurMusicPos / 1000.0f) * 5 * speedMultiplierCompensation *
                                 m_fposu_mod_strafing_frequency_y_ref->getFloat()) *
                        m_fposu_mod_strafing_strength_y_ref->getFloat();
        const float z = std::sin((m_iCurMusicPos / 1000.0f) * 5 * speedMultiplierCompensation *
                                 m_fposu_mod_strafing_frequency_z_ref->getFloat()) *
                        m_fposu_mod_strafing_strength_z_ref->getFloat();
    ...
    /*
    {
            ...
            if (depthMultiplier >= -1.0f)
            {
                    ...
                    float depthMultiplierClamped = clamp<float>(depthMultiplier, -1.0f, stopOvershootPercent); // -1 0
    stopOvershootPercent depthMultiplierClamped = (depthMultiplierClamped + 1.0f) / (1.0f); // 0 1+stopOvershootPercent

                    if (depthMultiplierClamped < 1.0f)
                    {
                            //depthMultiplierClamped = 1.0f - (1.0f - depthMultiplierClamped)*(1.0f -
    depthMultiplierClamped);
                            //depthMultiplierClamped = 1.0f - (1.0f - depthMultiplierClamped)*(1.0f -
    depthMultiplierClamped);
                            //depthMultiplierClamped = 1.0f - (1.0f - depthMultiplierClamped)*(1.0f -
    depthMultiplierClamped);
                    ...
                    {
                            //depthMultiplierClamped = 1.0f - (1.0f - (depthMultiplierClamped - 1.0f)) * (1.0f -
    (depthMultiplierClamped - 1.0f));
                    }
    ...
    */
    ...
    /*
    {
            // TODO: try coords.y multiplier such that we get 3d multiplied sines with random mountains sprinkled
    regularly, should be more enjoyable to have more height differences like that coords3d.z +=
    std::sin(m_iCurMusicPos/1000.0f*3 + (coords.x / (float)OsuGameRules::OSU_COORD_WIDTH)*15)*0.2f;
    }
    */

    if(m_fposu_curved_ref->getBool())
        return (coords3d + Vector3(0, 0,
                                   -quadLerp3f(m_fposu_distance_ref->getFloat(), m_osu->getFPoSu()->getEdgeDistance(),
                                               m_fposu_distance_ref->getFloat(), xCurvePercent)));
    else
