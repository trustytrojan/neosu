...
OsuUISongBrowserScoreButton::OsuUISongBrowserScoreButton(Osu *osu, OsuUIContextMenu *contextMenu, float xPos,
                                                         float yPos, float xSize, float ySize, STYLE style)
...
void OsuUISongBrowserScoreButton::onDeleteScoreClicked() {
    if(m_contextMenu != NULL) {
        ...
        m_contextMenu->setClickCallback(
            fastdelegate::MakeDelegate(this, &OsuUISongBrowserScoreButton::onDeleteScoreConfirmed));
        OsuUIContextMenu::clampToRightScreenEdge(m_contextMenu);
        OsuUIContextMenu::clampToBottomScreenEdge(m_contextMenu);
    }
}
...
void OsuUISongBrowserScoreButton::setScore(const OsuDatabase::Score &score, const OsuDatabaseBeatmap *diff2, int index,
    ...
    if(diff2 != NULL && !score.isImportedLegacyScore && !score.isLegacyScore) {
        const OsuReplay::BEATMAP_VALUES beatmapValuesForModsLegacy = OsuReplay::getBeatmapValuesForModsLegacy(
        ...
        const float compensatedAR = std::round(OsuGameRules::getRawApproachRateForSpeedMultiplier(
                                                   OsuGameRules::getRawApproachTime(score.AR), score.speedMultiplier) *
                                               100.0f) /
                                    100.0f;
        const float compensatedOD = std::round(OsuGameRules::getRawOverallDifficultyForSpeedMultiplier(
