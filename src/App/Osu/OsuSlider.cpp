...
OsuSlider::OsuSlider(char type, int repeat, float pixelLength, std::vector<Vector2> points, std::vector<int> hitSounds,
    ...
    if(m_osu_mod_fps_ref == NULL) m_osu_mod_fps_ref = convar->getConVarByName("osu_mod_fps");
...
void OsuSlider::draw(Graphics *g) {
                               ...
                               // fadeout animation below this if block
    {
        ...
        // draw slider body
        if(alpha > 0.0f && osu_slider_draw_body.getBool()) drawBody(g, alpha, sliderSnakeStart, sliderSnake);
    ...
    if(m_fStartHitAnimation > 0.0f && m_fStartHitAnimation != 1.0f && !m_beatmap->getOsu()->getModHD()) {
        ...
        g->pushTransform();
        {
            ...
            if(m_iCurRepeat < 1) {
                ...
                m_beatmap->getSkin()->getSliderStartCircleOverlay2()->setAnimationTimeOffset(
                    !m_beatmap->isInMafhamRenderChunk() ? m_iTime - m_iApproachTime
                                                        : m_beatmap->getCurMusicPosWithOffsets());
                ...
                m_beatmap->getSkin()->getSliderEndCircleOverlay2()->setAnimationTimeOffset(
                    !m_beatmap->isInMafhamRenderChunk() ? m_iTime : m_beatmap->getCurMusicPosWithOffsets());
    ...
    if(m_fEndHitAnimation > 0.0f && m_fEndHitAnimation != 1.0f && !m_beatmap->getOsu()->getModHD()) {
        ...
        g->pushTransform();
        {
            ...
            {
                m_beatmap->getSkin()->getHitCircleOverlay2()->setAnimationTimeOffset(
                    !m_beatmap->isInMafhamRenderChunk() ? m_iTime - m_iFadeInTime
                                                        : m_beatmap->getCurMusicPosWithOffsets());
                m_beatmap->getSkin()->getSliderEndCircleOverlay2()->setAnimationTimeOffset(
                    !m_beatmap->isInMafhamRenderChunk() ? m_iTime - m_iFadeInTime
                                                        : m_beatmap->getCurMusicPosWithOffsets());
                               ...
                               // delay and the fadeout animation below this if-block
    {
        if(m_fSlidePercent > 0.0f) {
            ...
            g->setColor(skin->getAllowSliderBallTint()
                            ? (osu_slider_ball_tint_combo_color.getBool()
...
void OsuSlider::drawVR(Graphics *g, Matrix4 &mvp, OsuVR *vr) {
    ...
    /*
    if (m_fEndSliderBodyFadeAnimation > 0.0f && m_fEndSliderBodyFadeAnimation != 1.0f &&
    !m_beatmap->getOsu()->getModHD() && !osu_slider_shrink.getBool())
    {
        ...
        for(int t = 0; t < m_ticks.size(); t++) {
            ...
            g->pushTransform();
            {
                Matrix4 modelMatrix;
                {
                    ...
                    scale.scale((skin->getSliderScorePoint()->getSize().x /
                                 (16.0f * (skin->isSliderScorePoint2x() ? 2.0f : 1.0f))) *
                                0.125f);
                    ...
                    if(m_fposu_3d_hitobjects_look_at_player_ref->getBool())
                        modelMatrix =
                            translation *
                            Camera::buildMatrixLookAt(Vector3(0, 0, 0),
                                                      pos - m_beatmap->getOsu()->getFPoSu()->getCamera()->getPos(),
                                                      Vector3(0, 1, 0))
                                .invert() *
                            scale;
                    else
        ...
        if(m_points.size() > 1) {
            ...
            if(m_fReverseArrowAlpha > 0.0f) {
                ...
                if(m_iReverseArrowPos == 2 || m_iReverseArrowPos == 3) {
                    ...
                    float reverseArrowImageScale =
                        skin->getReverseArrow()->getSize().x / (128.0f * (skin->isReverseArrow2x() ? 2.0f : 1.0f));
                    ...
                    g->pushTransform();
                    {
                        Matrix4 modelMatrix;
                        {
                            ...
                            if(m_fposu_3d_hitobjects_look_at_player_ref->getBool())
                                modelMatrix =
                                    translation *
                                    Camera::buildMatrixLookAt(
                                        Vector3(0, 0, 0), pos - m_beatmap->getOsu()->getFPoSu()->getCamera()->getPos(),
                                        Vector3(0, 1, 0))
                                        .invert() *
                                    scale * rotate;
                            else
    ...
    if(!m_fposu_3d_spheres_ref->getBool()) {
        ...
        !m_beatmap->getOsu()->getModHD())
        {
                ...
                if (!osu_slider_shrink.getBool())
                        ...
                        OsuSliderRenderer::draw(g, m_beatmap->getOsu(), emptyVector, alwaysPoints,
        m_beatmap->getHitcircleDiameter(), 0.0f, 0.0f, m_beatmap->getSkin()->getComboColorForCounter(m_iColorCounter,
        m_iColorOffset), 1.0f, 1.0f - m_fEndSliderBodyFadeAnimation, getTime());
        }
        */
        ...
        if(m_fStartHitAnimation > 0.0f && m_fStartHitAnimation != 1.0f && !m_beatmap->getOsu()->getModHD()) {
            ...
            if(m_iCurRepeat < 1) {
                ...
                OsuCircle::draw3DSliderStartCircle(g, m_beatmap, this, baseScale, m_curve->pointAt(0.0f),
                                                   m_iComboNumber, m_iColorCounter, m_iColorOffset, 1.0f, 1.0f, alpha,
                                                   alpha, drawNumber);
            } else {
                m_beatmap->getSkin()->getHitCircleOverlay2()->setAnimationTimeOffset(
                    !m_beatmap->isInMafhamRenderChunk() ? m_iTime : m_beatmap->getCurMusicPosWithOffsets());
                ...
                OsuCircle::draw3DSliderEndCircle(g, m_beatmap, this, baseScale, m_curve->pointAt(0.0f), m_iComboNumber,
                                                 m_iColorCounter, m_iColorOffset, 1.0f, 1.0f, alpha, alpha, drawNumber);
            }
        }
        ...
        if(m_fEndHitAnimation > 0.0f && m_fEndHitAnimation != 1.0f && !m_beatmap->getOsu()->getModHD()) {
            ...
            OsuCircle::draw3DSliderEndCircle(g, m_beatmap, this, baseScale, m_curve->pointAt(1.0f), m_iComboNumber,
                                             m_iColorCounter, m_iColorOffset, 1.0f, 1.0f, alpha, 0.0f, false);
        }
    }
...
void OsuSlider::draw3D2(Graphics *g) {
                                            ...
                                            // and the fadeout animation below this if block
    {
        if(m_points.size() > 1) {
            ...
            // start circle
            if(!m_bStartFinished || !sliderRepeatStartCircleFinished || (!m_bEndFinished && m_iRepeat % 2 == 0)) {
                OsuCircle::draw3DApproachCircle(g, m_beatmap, this, baseScale, m_curve->pointAt(0.0f), m_iComboNumber,
                                                m_iColorCounter, m_iColorOffset,
    ...
    if((m_bVisible && m_bCursorInside &&
       ...
       (m_bFinished && m_fFollowCircleAnimationAlpha > 0.0f && m_bHeldTillEnd)) {
        ...
        g->pushTransform();
        {
            ...
            {
                Matrix4 scale = baseScale;
                scale.scale((m_beatmap->getRawSliderFollowCircleDiameter() / m_beatmap->getRawHitcircleDiameter()) *
                            (skin->getSliderFollowCircle2()->getImageSizeForCurrentFrame().x /
                             skin->getSliderFollowCircle2()->getSizeBaseRaw().x) *
                            tickAnimationScale * m_fFollowCircleAnimationScale * 0.85f);
                ...
                if(m_fposu_3d_hitobjects_look_at_player_ref->getBool())
                    modelMatrix = translation *
                                  Camera::buildMatrixLookAt(
                                      Vector3(0, 0, 0), point - m_beatmap->getOsu()->getFPoSu()->getCamera()->getPos(),
                                      Vector3(0, 1, 0))
                                      .invert() *
                                  scale;
                else
                               ...
                               // delay and the fadeout animation below this if-block
    {
        if(m_fSlidePercent > 0.0f) {
            ...
            g->pushTransform();
            {
                ...
                {
                    Matrix4 scale = baseScale;
                    scale.scale(skin->getSliderb()->getImageSizeForCurrentFrame().x /
                                skin->getSliderb()->getSizeBaseRaw().x);
                    ...
                    if(m_fposu_3d_hitobjects_look_at_player_ref->getBool())
                        modelMatrix =
                            translation *
                            Camera::buildMatrixLookAt(Vector3(0, 0, 0),
                                                      point - m_beatmap->getOsu()->getFPoSu()->getCamera()->getPos(),
                                                      Vector3(0, 1, 0))
                                .invert() *
                            scale * rotate;
                    else
...
void OsuSlider::draw3DStartCircle(Graphics *g, const Matrix4 &baseScale, float alpha) {
    if(m_bStartFinished) {
        ...
        OsuCircle::draw3DSliderEndCircle(g, m_beatmap, this, baseScale, m_curve->pointAt(0.0f), m_iComboNumber,
                                         m_iColorCounter, m_iColorOffset, m_fHittableDimRGBColorMultiplierPercent, 1.0f,
                                         m_fAlpha, 0.0f, false, false);
    } else {
        m_beatmap->getSkin()->getHitCircleOverlay2()->setAnimationTimeOffset(
            !m_beatmap->isInMafhamRenderChunk() ? m_iTime - m_iApproachTime : m_beatmap->getCurMusicPosWithOffsets());
        m_beatmap->getSkin()->getSliderStartCircleOverlay2()->setAnimationTimeOffset(
        ...
        OsuCircle::draw3DSliderStartCircle(g, m_beatmap, this, baseScale, m_curve->pointAt(0.0f), m_iComboNumber,
                                           m_iColorCounter, m_iColorOffset, m_fHittableDimRGBColorMultiplierPercent,
                                           m_fApproachScale, m_fAlpha, m_fAlpha, !m_bHideNumberAfterFirstRepeatHit,
                                           m_bOverrideHDApproachCircle);
    }
}
...
void OsuSlider::draw3DEndCircle(Graphics *g, const Matrix4 &baseScale, float alpha, float sliderSnake) {
    ...
    OsuCircle::draw3DSliderEndCircle(g, m_beatmap, this, baseScale, m_curve->pointAt(sliderSnake), m_iComboNumber,
                                     m_iColorCounter, m_iColorOffset, m_fHittableDimRGBColorMultiplierPercent, 1.0f,
                                     m_fAlpha, 0.0f, false, false);
}
...
void OsuSlider::update(long curPos) {
    ...
    if(!m_bEndFinished) {
        ...
        const bool isTrackingStrictTrackingMod =
            ((m_bStartFinished || curPos >= m_iTime) && m_osu_mod_strict_tracking_ref->getBool());
...
void OsuSlider::updateAnimations(long curPos) {
    ...
    m_fFollowCircleAnimationAlpha =
        clamp<float>((float)((curPos - m_iTime)) / 1000.0f /
                         clamp<float>(OsuGameRules::osu_slider_followcircle_fadein_fade_time.getFloat(), 0.0f,
                                      m_iObjectDuration / 1000.0f),
                     0.0f, 1.0f);
    if(m_bFinished) {
        m_fFollowCircleAnimationAlpha =
            1.0f - clamp<float>((float)((curPos - (m_iTime + m_iObjectDuration))) / 1000.0f /
                                    OsuGameRules::osu_slider_followcircle_fadeout_fade_time.getFloat(),
                                0.0f, 1.0f);
    ...
    m_fFollowCircleAnimationScale =
        clamp<float>((float)((curPos - m_iTime)) / 1000.0f /
                         clamp<float>(OsuGameRules::osu_slider_followcircle_fadein_scale_time.getFloat(), 0.0f,
                                      m_iObjectDuration / 1000.0f),
                     0.0f, 1.0f);
    if(m_bFinished) {
        m_fFollowCircleAnimationScale =
            clamp<float>((float)((curPos - (m_iTime + m_iObjectDuration))) / 1000.0f /
                             OsuGameRules::osu_slider_followcircle_fadeout_scale_time.getFloat(),
                         0.0f, 1.0f);
    }
    m_fFollowCircleAnimationScale =
...
void OsuSlider::onRepeatHit(bool successful, bool sliderend) {
    ...
    if(!successful)
        ...
        m_fFollowCircleTickAnimationScale = 0.0f;
        anim->moveLinear(&m_fFollowCircleTickAnimationScale, 1.0f,
                         OsuGameRules::osu_slider_followcircle_tick_pulse_time.getFloat(), true);
