...
void OsuCircle::drawApproachCircle(Graphics *g, OsuBeatmapStandard *beatmap, Vector2 rawPos, int number,
    ...
    comboColor =
        COLOR(255, (int)(COLOR_GET_Ri(comboColor) * colorRGBMultiplier * osu_circle_color_saturation.getFloat()),
              (int)(COLOR_GET_Gi(comboColor) * colorRGBMultiplier * osu_circle_color_saturation.getFloat()),
...
void OsuCircle::draw3DApproachCircle(Graphics *g, OsuBeatmapStandard *beatmap, const OsuHitObject *hitObject,
                                     const Matrix4 &baseScale, Vector2 rawPos, int number, int colorCounter,
    ...
    draw3DApproachCircle(g, beatmap->getOsu()->getFPoSu(), baseScale, beatmap->getSkin(),
                         beatmap->osuCoordsTo3D(rawPos, hitObject), comboColor, beatmap->getRawHitcircleDiameter(),
                         approachScale, alpha, beatmap->getOsu()->getModHD(), overrideHDApproachCircle);
}
...
void OsuCircle::draw3DCircle(Graphics *g, OsuBeatmapStandard *beatmap, const OsuHitObject *hitObject,
                             const Matrix4 &baseScale, Vector2 rawPos, int number, int colorCounter, int colorOffset,
                             float colorRGBMultiplier, float approachScale, float alpha, float numberAlpha,
                             bool drawNumber, bool overrideHDApproachCircle) {
    draw3DCircle(g, beatmap->getOsu()->getFPoSu(), baseScale, beatmap->getSkin(),
                 beatmap->osuCoordsTo3D(rawPos, hitObject), beatmap->getRawHitcircleDiameter(),
                 beatmap->getNumberScale(), beatmap->getHitcircleOverlapScale(), number, colorCounter, colorOffset,
                 colorRGBMultiplier, approachScale, alpha, numberAlpha, drawNumber, overrideHDApproachCircle);
}
...
void OsuCircle::draw3DCircle(Graphics *g, OsuModFPoSu *fposu, const Matrix4 &baseScale, OsuSkin *skin, Vector3 pos,
                             float rawHitcircleDiameter, float numberScale, float overlapScale, int number,
                             int colorCounter, int colorOffset, float colorRGBMultiplier, float approachScale,
    ...
    // approach circle
    /// draw3DApproachCircle(g, fposu, baseScale, skin, pos, comboColor, rawHitcircleDiameter, approachScale, alpha,
    /// modHD, overrideHDApproachCircle); // they are now drawn separately in draw3D2()
...
void OsuCircle::draw3DSliderStartCircle(Graphics *g, OsuBeatmapStandard *beatmap, const OsuHitObject *hitObject,
                                        const Matrix4 &baseScale, Vector2 rawPos, int number, int colorCounter,
                                        int colorOffset, float colorRGBMultiplier, float approachScale, float alpha,
                                        float numberAlpha, bool drawNumber, bool overrideHDApproachCircle) {
    draw3DSliderStartCircle(g, beatmap->getOsu()->getFPoSu(), baseScale, beatmap->getSkin(),
                            beatmap->osuCoordsTo3D(rawPos, hitObject), beatmap->getRawHitcircleDiameter(),
                            beatmap->getNumberScale(), beatmap->getHitcircleOverlapScale(), number, colorCounter,
                            colorOffset, colorRGBMultiplier, approachScale, alpha, numberAlpha, drawNumber,
                            overrideHDApproachCircle);
...
void OsuCircle::draw3DSliderStartCircle(Graphics *g, OsuModFPoSu *fposu, const Matrix4 &baseScale, OsuSkin *skin,
                                        Vector3 pos, float rawHitcircleDiameter, float numberScale,
    ...
    // if no sliderstartcircle image is preset, fallback to default circle
    if(skin->getSliderStartCircle() == skin->getMissingTexture()) {
        draw3DCircle(g, fposu, baseScale, skin, pos, rawHitcircleDiameter, numberScale, hitcircleOverlapScale, number,
                     colorCounter, colorOffset, colorRGBMultiplier, approachScale, alpha, numberAlpha, drawNumber,
                     overrideHDApproachCircle);  // normal
        return;
    }
    ...
    // approach circle
    /// drawApproachCircle(g, skin, pos, comboColor, beatmap->getHitcircleDiameter(), approachScale, alpha,
    /// beatmap->getOsu()->getModHD(), overrideHDApproachCircle); // they are now drawn separately in draw3D2()
    ...
    // number
    if(drawNumber)
        draw3DHitCircleNumber(g, skin, numberScale, hitcircleOverlapScale, pos, number, numberAlpha,
                              colorRGBMultiplier);
...
void OsuCircle::draw3DSliderEndCircle(Graphics *g, OsuBeatmapStandard *beatmap, const OsuHitObject *hitObject,
                                      const Matrix4 &baseScale, Vector2 rawPos, int number, int colorCounter,
                                      int colorOffset, float colorRGBMultiplier, float approachScale, float alpha,
                                      float numberAlpha, bool drawNumber, bool overrideHDApproachCircle) {
    draw3DSliderEndCircle(g, beatmap->getOsu()->getFPoSu(), baseScale, beatmap->getSkin(),
                          beatmap->osuCoordsTo3D(rawPos, hitObject), beatmap->getRawHitcircleDiameter(),
                          beatmap->getNumberScale(), beatmap->getHitcircleOverlapScale(), number, colorCounter,
                          colorOffset, colorRGBMultiplier, approachScale, alpha, numberAlpha, drawNumber,
                          overrideHDApproachCircle);
...
void OsuCircle::draw3DSliderEndCircle(Graphics *g, OsuModFPoSu *fposu, const Matrix4 &baseScale, OsuSkin *skin,
                                      Vector3 pos, float rawHitcircleDiameter, float numberScale, float overlapScale,
    ...
    // if no sliderendcircle image is preset, fallback to default circle
    if(skin->getSliderEndCircle() == skin->getMissingTexture()) {
        draw3DCircle(g, fposu, baseScale, skin, pos, rawHitcircleDiameter, numberScale, overlapScale, number,
                     colorCounter, colorOffset, colorRGBMultiplier, approachScale, alpha, numberAlpha, drawNumber,
                     overrideHDApproachCircle);
        return;
    }
...
void OsuCircle::draw3DApproachCircle(Graphics *g, OsuModFPoSu *fposu, const Matrix4 &baseScale, OsuSkin *skin,
                                     Vector3 pos, Color comboColor, float rawHitcircleDiameter, float approachScale,
                                     float alpha, bool modHD, bool overrideHDApproachCircle) {
    if((!modHD || overrideHDApproachCircle) && osu_draw_approach_circles.getBool() &&
       !OsuGameRules::osu_mod_mafham.getBool()) {
        if(approachScale > 1.0f) {
            const float approachCircleImageScale =
                skin->getApproachCircle()->getSize().x / (128.0f * (skin->isApproachCircle2x() ? 2.0f : 1.0f));
            ...
            g->pushTransform();
            {
                Matrix4 modelMatrix;
                {
                    ...
                    if(m_fposu_3d_hitobjects_look_at_player_ref->getBool() &&
                       m_fposu_3d_approachcircles_look_at_player_ref->getBool())
                        modelMatrix = translation *
                                      Camera::buildMatrixLookAt(Vector3(0, 0, 0), pos - fposu->getCamera()->getPos(),
                                                                Vector3(0, 1, 0))
                                          .invert() *
                                      scale;
                    else
...
void OsuCircle::draw3DHitCircleOverlay(Graphics *g, OsuModFPoSu *fposu, const Matrix4 &baseScale,
                                       OsuSkinImage *hitCircleOverlayImage, Vector3 pos, float alpha,
    ...
    g->pushTransform();
    {
        Matrix4 modelMatrix;
        {
            Matrix4 scale = baseScale;
            scale.scale(
                (hitCircleOverlayImage->getImageSizeForCurrentFrame().x / hitCircleOverlayImage->getSizeBaseRaw().x));
            ...
            if(m_fposu_3d_hitobjects_look_at_player_ref->getBool())
                modelMatrix =
                    translation *
                    Camera::buildMatrixLookAt(Vector3(0, 0, 0), pos - fposu->getCamera()->getPos(), Vector3(0, 1, 0))
                        .invert() *
                    scale;
            else
                modelMatrix = translation * scale;
...
void OsuCircle::draw3DHitCircle(Graphics *g, OsuModFPoSu *fposu, OsuSkin *skin, const Matrix4 &baseScale,
                                Image *hitCircleImage, Vector3 pos, Color comboColor, float alpha) {
    g->setColor(comboColor);
    ...
    g->pushTransform();
    {
        if(m_fposu_3d_spheres_ref->getBool()) {
            ...
            fposu->getHitcircleShader()->enable();
            {
                ...
                fposu->getHitcircleShader()->setUniformMatrix4fv("modelMatrixInverseTransposed",
                                                                 modelMatrixInverseTransposed);
...
void OsuCircle::drawHitCircleNumber(Graphics *g, OsuSkin *skin, float numberScale, float overlapScale, Vector2 pos,
    ...
    g->pushTransform();
    {
        ...
        for(int i = digits.size() - 1; i >= 0; i--) {
            ...
            g->translate((DigitWidth::getWidth(skin, digits[i]) * numberScale +
                          DigitWidth::getWidth(skin, digits[i - 1]) * numberScale) *
                                 0.5f -
                             skin->getHitCircleOverlap() * overlapScale,
                         0);
        }
    }
    g->popTransform();
...
void OsuCircle::draw3DHitCircleNumber(Graphics *g, OsuSkin *skin, float numberScale, float overlapScale, Vector3 pos,
                                      int number, float numberAlpha, float colorRGBMultiplier) {
    if(m_fposu_3d_spheres_ref->getBool()) return;
...
void OsuCircle::draw(Graphics *g) {
    ...
    {
        float smooth = 1.0f - ((m_fShakeAnimation - engine->getTime()) /
                               osu_circle_shake_duration.getFloat());  // goes from 0 to 1
        if(smooth < 0.5f)
            smooth = smooth / 0.5f;
        else
...
void OsuCircle::draw2(Graphics *g) {
    ...
    if(osu_bug_flicker_log.getBool()) {
        const float approachCircleImageScale =
            m_beatmap->getHitcircleDiameter() / (128.0f * (m_beatmap->getSkin()->isApproachCircle2x() ? 2.0f : 1.0f));
        debugLog("m_iTime = %ld, aScale = %f, iScale = %f\n", m_iTime, m_fApproachScale, approachCircleImageScale);
    }

    drawApproachCircle(g, m_beatmap, m_vRawPos, m_iComboNumber, m_iColorCounter, m_iColorOffset,
                       m_fHittableDimRGBColorMultiplierPercent, m_bWaiting && !hd ? 1.0f : m_fApproachScale,
...
void OsuCircle::draw3D(Graphics *g) {
    ...
    if(!m_fposu_3d_spheres_ref->getBool()) {
        if(m_fHitAnimation > 0.0f && m_fHitAnimation != 1.0f && !m_beatmap->getOsu()->getModHD()) {
            ...
            m_beatmap->getSkin()->getHitCircleOverlay2()->setAnimationTimeOffset(
                !m_beatmap->isInMafhamRenderChunk() ? m_iTime - m_iApproachTime
                                                    : m_beatmap->getCurMusicPosWithOffsets());
            draw3DCircle(g, m_beatmap, this, baseScale, m_vRawPos, m_iComboNumber, m_iColorCounter, m_iColorOffset,
    ...
    m_beatmap->getSkin()->getHitCircleOverlay2()->setAnimationTimeOffset(
        !m_beatmap->isInMafhamRenderChunk() ? m_iTime - m_iApproachTime : m_beatmap->getCurMusicPosWithOffsets());
    draw3DCircle(g, m_beatmap, this, baseScale, shakeCorrectedRawPos, m_iComboNumber, m_iColorCounter, m_iColorOffset,
                 (m_fposu_3d_spheres_ref->getBool() ? 1.0f : m_fHittableDimRGBColorMultiplierPercent),
                 m_bWaiting && !hd ? 1.0f : m_fApproachScale, m_bWaiting && !hd ? 1.0f : m_fAlpha,
                 m_bWaiting && !hd ? 1.0f : m_fAlpha, true, m_bOverrideHDApproachCircle);
}
...
void OsuCircle::draw3D2(Graphics *g) {
    ...
    draw3DApproachCircle(g, m_beatmap, this, baseScale, m_vRawPos, m_iComboNumber, m_iColorCounter, m_iColorOffset,
                         m_fHittableDimRGBColorMultiplierPercent, m_bWaiting && !hd ? 1.0f : m_fApproachScale,
                         m_bWaiting && !hd ? 1.0f : m_fAlphaForApproachCircle, m_bOverrideHDApproachCircle);
}
