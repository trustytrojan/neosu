...
class OsuUserStatsScreenMenuButton : public CBaseUIButton {
   public:
    OsuUserStatsScreenMenuButton(float xPos = 0, float yPos = 0, float xSize = 0, float ySize = 0, UString name = "",
                                 UString text = "")
        ...
        for(auto &kv : *scores) {
            for(size_t i = 0; i < kv.second.size(); i++) {
                ...
                if((!score.isLegacyScore || m_bImportLegacyScores) && score.playerName == sUsername) {
                    ...
                    if(!OsuDatabaseBeatmap::loadMetadata(diff2)) continue;
                    ...
                    int maxPossibleCombo = 0;
                    {
                        ...
                        pp = OsuDifficultyCalculator::calculatePPv2(
...
OsuUserStatsScreen::OsuUserStatsScreen(Osu *osu) : OsuScreenBackable(osu) {
    ...
    {
        UString iconString;
        iconString.insert(0, OsuIcons::GEAR);
...
void OsuUserStatsScreen::rebuildScoreButtons(UString playerName) {
    ...
    for(int i = scores.size() - 1; i >= std::max(0, (int)scores.size() - osu_ui_top_ranks_max.getInt()); i--) {
        ...
        OsuUISongBrowserScoreButton *button = new OsuUISongBrowserScoreButton(
            m_osu, m_contextMenu, 0, 0, 300, 100, OsuUISongBrowserScoreButton::STYLE::TOP_RANKS);
...
void OsuUserStatsScreen::onRecalculatePPImportLegacyScoresClicked() {
    ...
    m_contextMenu->setClickCallback(
        fastdelegate::MakeDelegate(this, &OsuUserStatsScreen::onRecalculatePPImportLegacyScoresConfirmed));
    OsuUIContextMenu::clampToRightScreenEdge(m_contextMenu);
}
...
void OsuUserStatsScreen::onRecalculatePP(bool importLegacyScores) {
    ...
    m_backgroundPPRecalculator =
        new OsuUserStatsScreenBackgroundPPRecalculator(m_osu, m_name_ref->getString(), importLegacyScores);

    // NOTE: force disable all runtime mods (including all experimental mods!), as they directly influence global
    // OsuGameRules which are used during pp calculation
