...
class OsuSongBrowserBackgroundSearchMatcher : public Resource {
   public:
    ...
    void setSongButtonsAndSearchString(const std::vector<OsuUISongBrowserSongButton *> &songButtons,
...
void OsuSongBrowser2::draw(Graphics *g) {
    ...
    */
        ...
        if(aimStrains.size() > 0 && aimStrains.size() == speedStrains.size()) {
            ...
            if(highestAimStrain > 0.0 && highestSpeedStrain > 0.0 && highestStrain > 0.0) {
                ...
                g->pushTransform();
                {
                        ...
                        g->setColor(0xffffffff);
                        g->drawString(m_osu->getSubTitleFont(), UString::format("avg strain = %i%% (%f)",
                (int)((averageStrain / highestStrain) * 100.0f), averageStrain)); g->translate(0,
                m_osu->getSubTitleFont()->getHeight()); g->drawString(m_osu->getSubTitleFont(), UString::format("top
                strain = %i%% (%f)", (int)((highestStrain / highestStrain) * 100.0f), highestStrain)); g->translate(0,
                m_osu->getSubTitleFont()->getHeight()); g->drawString(m_osu->getSubTitleFont(), UString::format("delta =
                %i%% (%f)", (int)(((highestStrain - averageStrain) / highestStrain) * 100.0f), (highestStrain -
                averageStrain)));
                }
                g->popTransform();
                */
    ...
    if (true)
    {
            ...
            g->pushTransform();
            {
                    g->translate((int)(m_userButton->getPos().x + m_userButton->getSize().x + 10),
    (int)(m_userButton->getPos().y + m_userButton->getSize().y/2 + font->getHeight()/2)); g->drawString(font,
    topRanksInfoMessage);
            }
            g->popTransform();
    }
    ...
    */
    ...
    g->pushTransform();
            ...
            g->drawString(m_osu->getSongBrowserFont(), UString::format("res %i",
    engine->getResourceManager()->getNumResources())); g->translate(0, m_osu->getSongBrowserFont()->getHeight()*2); int
    numVisibleSongButtons = 0; std::vector<CBaseUIElement*> *elements =
    m_songBrowser->getContainer()->getAllBaseUIElementsPointer(); for (int i=0; i<elements->size(); i++)
            {
...
void OsuSongBrowser2::onKeyDown(KeyboardEvent &key) {
    ...
    if(m_bVisible && m_bBeatmapRefreshScheduled &&
       (key == KEY_ESCAPE || key == (KEYCODE)OsuKeyBindings::GAME_PAUSE.getInt())) {
        m_db->cancel();
        key.consume();
        return;
    }
    ...
    if(!engine->getKeyboard()->isAltDown() && key == KEY_DOWN) {
        ...
        if(selectedIndex > -1 && selectedIndex + 1 < elements.size()) {
            ...
            OsuUISongBrowserSongButton *songButton =
                dynamic_cast<OsuUISongBrowserSongButton *>(elements[nextSelectionIndex]);
            if(nextButton != NULL) {
        ...
        if(selectedIndex > -1 && selectedIndex - 1 > -1) {
            ...
            if(nextButton != NULL) {
                ...
                if(isCollectionButton && nextSelectionIndex - 1 > -1) {
                    ...
                    OsuUISongBrowserCollectionButton *nextCollectionButton =
                        dynamic_cast<OsuUISongBrowserCollectionButton *>(elements[nextSelectionIndex]);
    ...
    if(key == KEY_LEFT && !m_bLeft) {
        ...
        for(int i = elements.size() - 1; i >= 0; i--) {
            const OsuUISongBrowserSongDifficultyButton *diffButtonPointer =
                dynamic_cast<OsuUISongBrowserSongDifficultyButton *>(elements[i]);
            const OsuUISongBrowserCollectionButton *collectionButtonPointer =
                dynamic_cast<OsuUISongBrowserCollectionButton *>(elements[i]);
               ...
               (!jumpToNextGroup || collectionButtonPointer != NULL)) {
                m_bNextScrollToSongButtonJumpFixUseScrollSizeDelta = true;
                {
                    ...
                    if(!jumpToNextGroup || collectionButtonPointer == NULL) {
                        ...
                        OsuUISongBrowserCollectionButton *collectionButton =
                            dynamic_cast<OsuUISongBrowserCollectionButton *>(elements[i]);
...
void OsuSongBrowser2::onKeyUp(KeyboardEvent &key) {
    ...
    if(key == KEY_F1 || key == (KEYCODE)OsuKeyBindings::TOGGLE_MODSELECT.getInt()) m_bF1Pressed = false;
    if(key == KEY_F2 || key == (KEYCODE)OsuKeyBindings::RANDOM_BEATMAP.getInt()) m_bF2Pressed = false;
...
CBaseUIContainer *OsuSongBrowser2::setVisible(bool visible) {
    ...
    if(m_bVisible) {
        ...
        if(m_beatmaps.size() == 0) refreshBeatmaps();
...
void OsuSongBrowser2::onPlayEnd(bool quit) {
    ...
    if(!quit) {
        ...
        OsuUISongBrowserSongDifficultyButton *selectedSongDiffButton =
            dynamic_cast<OsuUISongBrowserSongDifficultyButton *>(findCurrentlySelectedSongButton());
...
void OsuSongBrowser2::onSelectionChange(OsuUISongBrowserButton *button, bool rebuild) {
    ...
    OsuUISongBrowserSongDifficultyButton *songDiffButtonPointer =
        dynamic_cast<OsuUISongBrowserSongDifficultyButton *>(button);
    OsuUISongBrowserCollectionButton *collectionButtonPointer =
        dynamic_cast<OsuUISongBrowserCollectionButton *>(button);
...
void OsuSongBrowser2::updateSongButtonLayout() {
    ...
    for(size_t i = 0; i < elements.size(); i++) {
        ...
        if(songButton != NULL) {
            const OsuUISongBrowserSongDifficultyButton *diffButtonPointer =
                dynamic_cast<OsuUISongBrowserSongDifficultyButton *>(songButton);
...
void OsuSongBrowser2::onDatabaseLoadingFinished() {
    ...
    // initialize all collection (grouped) buttons
    {
        // artist
        {
            // 0-9
            {
                OsuUISongBrowserCollectionButton *b =
                    new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50,
                                                         "", "0-9", std::vector<OsuUISongBrowserButton *>());
            ...
            for(size_t i = 0; i < 26; i++) {
                ...
                OsuUISongBrowserCollectionButton *b = new OsuUISongBrowserCollectionButton(
                    m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "", artistCollectionName,
                    std::vector<OsuUISongBrowserButton *>());
                m_artistCollectionButtons.push_back(b);
            }
            ...
            // Other
            {
                OsuUISongBrowserCollectionButton *b =
                    new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50,
                                                         "", "Other", std::vector<OsuUISongBrowserButton *>());
                m_artistCollectionButtons.push_back(b);
            }
        }
        ...
        // bpm
        {
            OsuUISongBrowserCollectionButton *b =
                new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "Under 60 BPM", std::vector<OsuUISongBrowserButton *>());
            m_bpmCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "Under 120 BPM", std::vector<OsuUISongBrowserButton *>());
            m_bpmCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "Under 180 BPM", std::vector<OsuUISongBrowserButton *>());
            m_bpmCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "Under 240 BPM", std::vector<OsuUISongBrowserButton *>());
            m_bpmCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "Under 300 BPM", std::vector<OsuUISongBrowserButton *>());
            m_bpmCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "Over 300 BPM", std::vector<OsuUISongBrowserButton *>());
            m_bpmCollectionButtons.push_back(b);
        }
        ...
        // length
        {
            OsuUISongBrowserCollectionButton *b =
                new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "1 minute or less", std::vector<OsuUISongBrowserButton *>());
            m_lengthCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "2 minutes or less", std::vector<OsuUISongBrowserButton *>());
            m_lengthCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "3 minutes or less", std::vector<OsuUISongBrowserButton *>());
            m_lengthCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "4 minutes or less", std::vector<OsuUISongBrowserButton *>());
            m_lengthCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "5 minutes or less", std::vector<OsuUISongBrowserButton *>());
            m_lengthCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "10 minutes or less", std::vector<OsuUISongBrowserButton *>());
            m_lengthCollectionButtons.push_back(b);
            b = new OsuUISongBrowserCollectionButton(m_osu, this, m_songBrowser, m_contextMenu, 250, 250, 200, 50, "",
                                                     "Over 10 minutes", std::vector<OsuUISongBrowserButton *>());
            m_lengthCollectionButtons.push_back(b);
        }
    ...
    if(m_selectedBeatmap != NULL) {
        for(size_t i = 0; i < m_visibleSongButtons.size(); i++) {
            if(m_visibleSongButtons[i]->getDatabaseBeatmap() == m_selectedBeatmap->getSelectedDifficulty2()) {
                OsuUISongBrowserSongButton *songButtonPointer =
                    dynamic_cast<OsuUISongBrowserSongButton *>(m_visibleSongButtons[i]);
                if(songButtonPointer != NULL) {
                    for(OsuUISongBrowserButton *diffButton : songButtonPointer->getChildren()) {
                        OsuUISongBrowserSongButton *diffButtonPointer =
                            dynamic_cast<OsuUISongBrowserSongButton *>(diffButton);
...
void OsuSongBrowser2::onSortChangeInt(UString text, bool autoScroll) {
    ...
    if(m_group == GROUP::GROUP_NO_GROUPING) {
        // HACKHACK: TODO: this needs to happen to support correct sorting, but using this requires proper regrouping
        // logic (with memory allocations every time)
        /*
...
void OsuSongBrowser2::onSelectionOptions() {
    ...
    if(currentlySelectedSongButton != NULL) {
        ...
        OsuUISongBrowserSongButton *songButtonPointer =
            dynamic_cast<OsuUISongBrowserSongButton *>(currentlySelectedSongButton);
        OsuUISongBrowserCollectionButton *collectionButtonPointer =
            dynamic_cast<OsuUISongBrowserCollectionButton *>(currentlySelectedSongButton);
        if(songButtonPointer != NULL)
...
void OsuSongBrowser2::recalculateStarsForSelectedBeatmap(bool force) {
    ...
    if(checkHandleKillDynamicStarCalculator(true)) {
        // NOTE: this only works safely because OsuDatabaseBeatmapStarCalculator does no work in load(), because it
...
void OsuSongBrowser2::selectRandomBeatmap(bool playMusicFromPreviewPoint) {
    ...
    for(size_t i = 0; i < elements.size(); i++) {
        ...
        OsuUISongBrowserSongDifficultyButton *songDifficultyButtonPointer =
            dynamic_cast<OsuUISongBrowserSongDifficultyButton *>(elements[i]);
