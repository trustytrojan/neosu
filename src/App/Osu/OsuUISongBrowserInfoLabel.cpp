...
OsuUISongBrowserInfoLabel::OsuUISongBrowserInfoLabel(Osu *osu, float xPos, float yPos, float xSize, float ySize,
                                                     UString name)
    ...
    if(isMouseInside() && !m_osu->getOptionsMenu()->isMouseInside()) {
        ...
        if(beatmap != NULL) {
            ...
            const float approachTimeRoundedCompensated =
                ((int)OsuGameRules::getApproachTime(beatmap)) * speedMultiplierInv;
            const float hitWindow300RoundedCompensated =
                ((int)OsuGameRules::getHitWindow300(beatmap) - 0.5f) * speedMultiplierInv;
            const float hitWindow100RoundedCompensated =
                ((int)OsuGameRules::getHitWindow100(beatmap) - 0.5f) * speedMultiplierInv;
            const float hitWindow50RoundedCompensated =
                ((int)OsuGameRules::getHitWindow50(beatmap) - 0.5f) * speedMultiplierInv;
            const float hitobjectRadiusRoundedCompensated =
                (OsuGameRules::getRawHitCircleDiameter(beatmap->getCS()) / 2.0f);
            ...
            m_osu->getTooltipOverlay()->begin();
            {
                ...
                m_osu->getTooltipOverlay()->addLine(
                    UString::format("Spinner difficulty: %.2f", OsuGameRules::getSpinnerSpinsPerSecond(beatmap)));
                m_osu->getTooltipOverlay()->addLine(
